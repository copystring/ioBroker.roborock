...
const unzipAsync = (0, util_1.promisify)(zlib.unzip);
...
class PhotoManager {
     ...
     */
    async handlePhotoProtocol300(duid, payloadBuf, isRoborockHeader) {
        if (!isRoborockHeader || payloadBuf.length < 16)
            return false;
        try {
            ...
            if (!this.adapter.pendingRequests.has(pendingReqId)) {
     ...
     */
    async handlePhotoProtocol301(duid, payloadBuf, isRoborockHeader) {
        let photoId = -1;
        let sequence = -1;
        let dataSkip = 24;
     ...
     */
    async isPhotoComplete(photoData, duid, photoId) {
        if (!photoData.totalSize || photoData.totalSize === 0)
            return false;
     ...
     */
    async extractPhotoData(rawPayload, duid, logId) {
