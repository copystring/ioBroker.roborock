...
class MapCreator {
    ...
    getRoomNameBySegment(segnum, mappedRooms, roomIDs) {
        if (!Array.isArray(mappedRooms) || !Array.isArray(roomIDs))
            return "Room " + segnum;
        ...
        if (typeof mappedIndex === "number") {
            if (roomIDs[mappedIndex] && roomIDs[mappedIndex].name)
                return roomIDs[mappedIndex].name;
            if (roomIDs[mappedIndex - 1] && roomIDs[mappedIndex - 1].name)
                return roomIDs[mappedIndex - 1].name;
        }
        return "Room " + segnum;
    }
        ...
        if (mapdata.IMAGE.dimensions.height > 0 && mapdata.IMAGE.dimensions.width > 0) {
            ...
            if (mapdata.CURRENTLY_CLEANED_ZONES) {
                if (mapdata.CURRENTLY_CLEANED_ZONES[0]) {
                    ...
                    mapdata.CURRENTLY_CLEANED_ZONES.forEach((coord) => {
                        ...
                        ctx.fillRect(this.robotXtoPixelX(mapdata.IMAGE, coord[0] / 50), this.robotYtoPixelY(mapdata.IMAGE, coord[1] / 50), this.robotXtoPixelX(mapdata.IMAGE, coord[2] / 50) - this.robotXtoPixelX(mapdata.IMAGE, coord[0] / 50), this.robotYtoPixelY(mapdata.IMAGE, coord[3] / 50) - this.robotYtoPixelY(mapdata.IMAGE, coord[1] / 50));
                        ctx.strokeStyle = "#2e8b57";
                        ctx.lineWidth = 4;
                        ctx.strokeRect(this.robotXtoPixelX(mapdata.IMAGE, coord[0] / 50), this.robotYtoPixelY(mapdata.IMAGE, coord[1] / 50), this.robotXtoPixelX(mapdata.IMAGE, coord[2] / 50) - this.robotXtoPixelX(mapdata.IMAGE, coord[0] / 50), this.robotYtoPixelY(mapdata.IMAGE, coord[3] / 50) - this.robotYtoPixelY(mapdata.IMAGE, coord[1] / 50));
                    });
                }
            }
            // Zeichne Teppich
            ...
            if (mapdata.FORBIDDEN_ZONES) {
                mapdata.FORBIDDEN_ZONES.forEach((zone) => {
                    ...
                    ctx.fillRect(this.robotXtoPixelX(mapdata.IMAGE, forbiddenMinX / 50), this.robotYtoPixelY(mapdata.IMAGE, forbiddenMaxY / 50), (forbiddenSizeX / 50) * this.adapter.config.map_scale, (forbiddenSizeY / 50) * this.adapter.config.map_scale);
                    ctx.lineWidth = (1 * this.adapter.config.map_scale) / 2;
                    ctx.strokeStyle = "rgba(255, 0, 0, 1)";
                    ctx.strokeRect(this.robotXtoPixelX(mapdata.IMAGE, forbiddenMinX / 50), this.robotYtoPixelY(mapdata.IMAGE, forbiddenMaxY / 50), (forbiddenSizeX / 50) * this.adapter.config.map_scale, (forbiddenSizeY / 50) * this.adapter.config.map_scale);
                });
            }
            ...
            if (mapdata.NO_MOP_ZONE) {
                mapdata.NO_MOP_ZONE.forEach((zone) => {
                    ...
                    ctx.fillRect(this.robotXtoPixelX(mapdata.IMAGE, noMopMinX / 50), this.robotYtoPixelY(mapdata.IMAGE, noMopMaxY / 50), (noMopSizeX / 50) * this.adapter.config.map_scale, (noMopSizeY / 50) * this.adapter.config.map_scale);
                    ctx.lineWidth = (1 * this.adapter.config.map_scale) / 2;
                    ctx.strokeStyle = "rgba(0, 0, 255, 1)";
                    ctx.strokeRect(this.robotXtoPixelX(mapdata.IMAGE, noMopMinX / 50), this.robotYtoPixelY(mapdata.IMAGE, noMopMaxY / 50), (noMopSizeX / 50) * this.adapter.config.map_scale, (noMopSizeY / 50) * this.adapter.config.map_scale);
                });
            }
            ...
            if (mapdata.CHARGER_LOCATION) {
                if (mapdata.CHARGER_LOCATION.position[0] && mapdata.CHARGER_LOCATION.position[1]) {
                    ...
                    ctx.drawImage(img_charger_rotated, this.robotXtoPixelX(mapdata.IMAGE, mapdata.CHARGER_LOCATION.position[0] / 50) - img_charger_rotated.width / 2, this.robotYtoPixelY(mapdata.IMAGE, mapdata.CHARGER_LOCATION.position[1] / 50) - img_charger_rotated.height / 2, img_charger_rotated.width, img_charger_rotated.height);
                }
            }
            // Zeichne Roboter
            if (mapdata.ROBOT_POSITION) {
                if (mapdata.PATH.current_angle && mapdata.ROBOT_POSITION[0] && mapdata.ROBOT_POSITION[1]) {
                    ...
                    ctx.drawImage(canvasImg, this.robotXtoPixelX(mapdata.IMAGE, mapdata.ROBOT_POSITION.position[0] / 50) - img.width / 4, this.robotYtoPixelY(mapdata.IMAGE, mapdata.ROBOT_POSITION.position[1] / 50) - img.height / 4, canvasImg.width / 2, canvasImg.height / 2);
                }
                else {
                    const img_robot_rotated = this.rotateCanvas(img, mapdata.ROBOT_POSITION.angle);
                    ctx.drawImage(img_robot_rotated, this.robotXtoPixelX(mapdata.IMAGE, mapdata.ROBOT_POSITION.position[0] / 50) - img_robot_rotated.width / 4, this.robotYtoPixelY(mapdata.IMAGE, mapdata.ROBOT_POSITION.position[1] / 50) - img_robot_rotated.height / 4, img_robot_rotated.width / 2, img_robot_rotated.height / 2);
                }
            }
            ...
            if (mapdata.GOTO_TARGET) {
                ...
                if (mapdata.GOTO_TARGET[0] && mapdata.GOTO_TARGET[1]) {
                    ctx.beginPath();
                    ctx.drawImage(go_to_pin_img, this.robotXtoPixelX(mapdata.IMAGE, mapdata.GOTO_TARGET[0] / 50) - go_to_pin_img.width / 2, this.robotYtoPixelY(mapdata.IMAGE, mapdata.GOTO_TARGET[1] / 50) - (go_to_pin_img.height + 6), go_to_pin_img.width, go_to_pin_img.height);
                }
            }
