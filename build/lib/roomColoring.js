...
exports.ROBOROCK_PALETTE = void 0;
...
function assignRoborockRoomColorsToHex(data, options) {
    ...
    const colorHex = new Array(matrixSize).fill(exports.ROBOROCK_PALETTE[0]);
    for (let i = idOffset; i < matrixSize; i++) {
        const colorIndex = colorData[i];
        if (colorIndex > 0 && colorIndex < exports.ROBOROCK_PALETTE.length) {
            colorHex[i] = exports.ROBOROCK_PALETTE[colorIndex];
        }
        else if (colorIndex !== 0) {
            // Fallback for indices outside range (wrap around 1-4)
            colorHex[i] = exports.ROBOROCK_PALETTE[1 + ((colorIndex - 1) % numColors)];
        }
    }
    return {
