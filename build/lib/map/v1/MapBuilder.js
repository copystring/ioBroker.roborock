...
class MapBuilder {
    ...
    createCarpetSprite() {
        ...
        const spriteCanvas = (0, canvas_1.createCanvas)(MapHelper_1.VISUAL_BLOCK_SIZE, MapHelper_1.VISUAL_BLOCK_SIZE);
        ...
        for (let dx = 0; dx < MapHelper_1.VISUAL_BLOCK_SIZE; dx++) {
            for (let dy = 0; dy < MapHelper_1.VISUAL_BLOCK_SIZE; dy++) {
                const sum = dx + dy;
    ...
    drawPathSegments(mainCtx, tempCtx, pathSegments, color, width, opacity, dashed = false) {
        ...
        if (dashed) {
            tempCtx.setLineDash([MapHelper_1.VISUAL_BLOCK_SIZE, 2 * MapHelper_1.VISUAL_BLOCK_SIZE]);
        }
        else {
            tempCtx.setLineDash([]);
        }
    ...
    async canvasMap(mapdata, params = {}) {
        ...
        mapdata.IMAGE.dimensions.width *= MapHelper_1.VISUAL_BLOCK_SIZE;
        mapdata.IMAGE.dimensions.height *= MapHelper_1.VISUAL_BLOCK_SIZE;
        ...
        if (image.pixels.floor && image.pixels.floor.length > 0) {
            ...
            ["floor", "obstacle"].forEach((key) => {
                ...
                image.pixels[key].forEach((px) => {
                    ...
                    ctx.rect(x, y, MapHelper_1.VISUAL_BLOCK_SIZE, MapHelper_1.VISUAL_BLOCK_SIZE);
        ...
        if (image.pixels.segments && this.colors.newmap) {
            ...
            Object.keys(segmentsData).forEach((segStr) => {
                ...
                segmentsData[segnum].points.forEach((p) => {
                    ctx.rect(p.x, p.y, MapHelper_1.VISUAL_BLOCK_SIZE, MapHelper_1.VISUAL_BLOCK_SIZE);
                });
                ctx.fill();
            });
        }
            ...
            image.pixels.segments.forEach((px) => {
                ...
                if (currentlyCleanedBlocks?.includes(segnum)) {
                    ...
                    px = px & 0xfffff;
                    ctx.rect(this.getX(image.dimensions, px), this.getY(image.dimensions, px), MapHelper_1.VISUAL_BLOCK_SIZE, MapHelper_1.VISUAL_BLOCK_SIZE);
                }
            });
            ctx.fill();
        }
    ...
    drawPaths(ctx, mapdata) {
        ...
        const lwMain = Math.max(1, MapHelper_1.VISUAL_BLOCK_SIZE / 2);
        const tempCanvas = (0, canvas_1.createCanvas)(ctx.canvas.width, ctx.canvas.height);
        const tempCtx = tempCanvas.getContext("2d");
        this.drawPathSegments(ctx, tempCtx, pathSegments.mopPath, "rgba(255, 255, 255, 1)", 6.5 * MapHelper_1.VISUAL_BLOCK_SIZE, 0.18);
        this.drawPathSegments(ctx, tempCtx, pathSegments.mainPath, MapHelper_1.LEGACY_COLORS.path, lwMain, 1.0);
    }
    ...
    drawPredictedPath(ctx, predictedPath, image) {
        if (predictedPath?.points?.length) {
            ctx.lineWidth = (3 * MapHelper_1.VISUAL_BLOCK_SIZE) / 2;
            ctx.strokeStyle = "rgba(255, 255, 255, 1)";
            ctx.setLineDash([3 * MapHelper_1.VISUAL_BLOCK_SIZE, 3 * MapHelper_1.VISUAL_BLOCK_SIZE]);
            ctx.lineCap = "round";
            ctx.beginPath();
            let lastX = -1, lastY = -1;
    ...
    async drawObstacles(ctx, obstacles, image, model) {
        ...
        if (obstacles) {
            for (const obstacle of obstacles) {
                ...
                const radius = MapHelper_1.VISUAL_BLOCK_SIZE * 3.5;
                ctx.beginPath();
                ...
                if (imagePath) {
                    try {
                        ...
                        const size = MapHelper_1.VISUAL_BLOCK_SIZE * 5;
        ...
        if (segmentsData && mappedRooms) {
            ...
            Object.keys(segmentsData).forEach((segnumStr) => {
                ...
                if (roomName) {
                    ...
                    ctx.font = `bold ${MapHelper_1.VISUAL_BLOCK_SIZE * 6}px Arial`;
    ...
    drawRestrictedAreas(ctx, mapdata) {
        const drawRectArea = (zones, fill, stroke) => {
            ...
            zones.forEach((zone) => {
                ...
                ctx.lineWidth = (1 * MapHelper_1.VISUAL_BLOCK_SIZE) / 2;
