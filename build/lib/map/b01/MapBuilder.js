...
class MapBuilder {
    ...
    async loadAssets(model, duid) {
        ...
        const attemptLoad = async (isRetry) => {
            ...
            try {
                ...
                const searchPaths = [];
                if (this.adapter)
                    this.adapter.rLog("MapManager", duid, "Debug", undefined, undefined, `loadAssets attempt (Retry=${isRetry}) for model: '${model}'`, "debug");
                ...
                const findAssetInPaths = async (candidates) => {
                    for (const dir of searchPaths) {
                        ...
                        for (const c of candidates) {
                            ...
                            if (fs.existsSync(p))
                                return await (0, canvas_1.loadImage)(p);
                        }
                        // 2. Check in subfolders (drawable-*, raw)
                        if (fs.existsSync(dir)) {
                            try {
                                const subdirs = fs.readdirSync(dir).filter(d => d.startsWith("drawable-") || d === "raw");
                ...
                if (chargerImg)
                    ...
                    if (isRetry && this.adapter)
                        this.adapter.rLog("MapManager", duid, "Error", undefined, undefined, "Charger asset NOT found.", "error");
                }
                ...
                if (Object.keys(this.assets.robot).length === 0) {
                    missingCritical = true;
                    if (isRetry && this.adapter)
                        this.adapter.rLog("MapManager", duid, "Error", undefined, undefined, "SEVERE ERROR: No Robot assets found!", "error");
                }
            }
            catch (e) {
                if (this.adapter)
    ...
    async buildMap(data, robotModel, duid, deviceStatus) {
        ...
        if (this.adapter)
            this.adapter.rLog("MapManager", duid, "Debug", "B01", undefined, startMsg, "debug");
        ...
        if (data.rooms) {
            data.rooms.forEach(r => {
                if (r.colorId !== undefined)
                    roomColorMap[r.roomId] = r.colorId;
            });
        }
        const safePalette = {
            WALL: (0, utils_1.hexToRgba)(constants_1.PALETTE.WALL),
            FLOOR: (0, utils_1.hexToRgba)(constants_1.PALETTE.FLOOR),
            CARPET: (0, utils_1.hexToRgba)(constants_1.PALETTE.CARPET),
            UNKNOWN: (0, utils_1.hexToRgba)(constants_1.PALETTE.UNKNOWN)
        };
        ...
        for (let i = 0; i < data.mapGrid.length; i++) {
            ...
            if (val >= 128) { // Wall
                ...
                if (!color)
                    color = safePalette.UNKNOWN;
                ctx.fillStyle = (0, utils_1.hexToRgba)(color);
                ctx.fillRect(x, cy, 1, 1);
            }
        }
        const toPixel = (wx, wy) => {
