...
class B01VacuumFeatures extends v1VacuumFeatures_1.V1VacuumFeatures {
    ...
    // Override updateStatus to use strict B01 prop.get
    async updateStatus() {
        const props = v1VacuumFeatures_1.V1VacuumFeatures.CONSTANTS.b01StatusProps;
        let resultObj;
        try {
    ...
    async updateConsumables(data) {
        ...
        if (data) {
            ...
            const props = v1VacuumFeatures_1.V1VacuumFeatures.CONSTANTS.b01SettingsProps;
            const result = await this.deps.adapter.requestsHandler.sendRequest(this.duid, "prop.get", { property: props });
            if (Array.isArray(result) && result.length === props.length) {
                resultObj = {};
                props.forEach((key, index) => {
    ...
    async processRecordAttributes(index, detail) {
        for (const key in detail) {
            ...
            const mappedKey = v1VacuumFeatures_1.V1VacuumFeatures.MAPPED_CLEAN_SUMMARY[key] || key;
            // Simple type mapping
