...
describe("AppPlugin desktop smart-home PoC", () => {
	...
	it("starts Q7 and Q10 from the real ignored local recordings instead of the synthetic map", () => {
		...
		expect(probe).toContain('profileId: options.profileId ?? "unknown"');
		expect(probe).toContain("availableProfiles: options.availableProfiles");
		expect(probe).toContain("const nativeHierarchy = options.runApplication");
		...
		expect(probe).toContain("nativeModuleImplementationCoverage");
		expect(probe).not.toContain("registry.assertImplements(registry.registeredModuleNames())");
		expect(probe).toContain("runtimeUsage: runtimeNativeUsage()");
		expect(source).toContain("runtimeUsage.missingNativeCallCount");
		expect(source).not.toContain("hostImplementation.implementedModuleCount");
		expect(source).toContain("appPluginRuntimeEvidence(this.mapSurface.snapshot())");
		expect(source).not.toContain("mapSource: this.mapSurface.snapshot()");
		expect(launcher).toContain("--session-descriptor");
