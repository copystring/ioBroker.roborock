...
Nur Punkt 5 beweist das vollständige Kartenverhalten. Phase 0 hat Punkt 1 für alle lokal vorhandenen Bundle-Dateien abgedeckt. Alle Metro-Bundles erreichen Punkt 2. Für das geprüfte Q10-X5+-Bundle sind zusätzlich Punkt 3 und Punkt 4 mit einer repräsentativen Karte und einem PNG-Golden nachgewiesen. Für Q7 L5 läuft das unveränderte Hermes-Bundle inzwischen im nativen Host; Raum-Tap, Auswahlfarbwechsel, Pinch-Zoom, der erfolgreiche Raumname-Happy-Path, die Leereingabe-Validierung und die AppPlugin-eigene Kürzung überlanger Raumnamen bis zur erzeugten Befehlsabsicht sind in derselben Sitzungsart nachgewiesen. Das ist ein Teil von Punkt 5, aber noch keine vollständige Familienfreigabe.
...
Damit sind der Rename-Happy-Path samt AppPlugin-eigener Payload-Bildung, die Blockierung eines leeren Raumnamens und der überlange Name bestanden. Beim leeren Namen bleibt der originale Dialog offen und das Bundle erzeugt keine Rename-Absicht. Beim überlangen Namen setzt der Q7-Raumdialog bewusst keine native `maxLength`-Prop: Sein eigenes `onChangeText` kürzt 26 eingegebene UTF-16-Codeeinheiten anhand des AppPlugin-Konstantenwerts `TEXTMAXLENGTH = 24`; erst dieser 24 Zeichen lange Name gelangt in `service.rename_room`. Das Limit und die Kürzung bleiben damit im unveränderten AppPlugin und werden nicht als Q7-Sonderregel im Host dupliziert. Noch offen bleiben vordefinierte und doppelte Namen, Gerätefehler, Timeout, Rollback und erneuter Kartenabruf sowie alle anderen Editierwerkzeuge.

Der reproduzierbare Gate-Lauf ist `npm run poc:appplugin-q7-rename-proof`. Er baut den Host-Probe neu und startet für gültigen, leeren und überlangen Namen jeweils eine frische Sitzung mit dem unveränderten Hermes-Bundle. Die versionierten Interaktionsfixtures liegen unter `test/fixtures/appplugin/`; lokale Gerätedaten, Schlüssel und Capture-only-Ergebnisse bleiben in ignorierten `artifacts/`. Der Gate-Runner prüft Bundle-Hash, vollständige Ereignisfolge, Pointer-Cleanup, AppPlugin-Ausnahmen, Antwort-Replay, exakt eine Rename-Absicht bei gültiger und gekürzter Eingabe sowie keine Rename-Absicht bei leerer Eingabe. Beim Längenfall wird zusätzlich bewiesen, dass der APK-Host den Text nicht kürzt, das AppPlugin ihn von 26 auf 24 Zeichen reduziert und sein Payload exakt diesen Wert enthält. Native-Aufrufe werden nach dem echten React-Native-Vertrag klassifiziert: Async-/Sync-/Bridge-Fehler und unerwartete Promise-Ablehnungen brechen den Lauf ab; die von der APK definierte `getFirmwareUpdateState`-Ablehnung `data is null` der leeren OTA-Fixture wird als erwartetes Promise-Ergebnis separat ausgewiesen.
...
1. Den automatisierten Q7-L5-Rename-Pfad um vordefinierte und doppelte Namen sowie Gerätefehler-, Timeout-, Rollback- und Kartenaktualisierungsfälle erweitern.
