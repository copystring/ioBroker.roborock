...
| `common_battery_percentage` | Процент заряда батареи |
| `common_cancel` | Отмена |
| `common_close_time` | Окончание |
| `common_custom_type` | Пользовательский тип |
...
| `fault_summery_503` | Установите контейнер для сбора пыли и фильтр на место.<br>Если проблема сохраняется, замените фильтр. |
...
| `guide_map_save_tip3` | После отключения функции «Сохранение карт» редактирование карты и настраиваемые функции уборки, такие как «Выборочная уборка комнат» и «Запретная зона», окажутся недоступны.<br> |
...
| `mapEdit_recommend_cill_subtitle` | <br>В этом месте обнаружен порог. Задайте зону порога. |
...
| `map_save_close_content` | После отключения функции «Сохранение карт» редактирование карты и настраиваемые функции уборки, такие как «Выборочная уборка комнат» и «Запретная зона», окажутся недоступны.<br> |
...
| `setting_carpet_avoid_tip` | После установки держателя протирочной салфетки робот будет избегать ковров и будет заезжать на них, только если это необходимо, чтобы не пропустить участок.<br>* Используйте эту функцию после добавления ковра в режиме редактирования карт |
...
| `setting_charging_note` | * Зарядка аккумулятора в пиковые часы возможна при следующих условиях:<br>1. При наличии незавершенных заданий.<br>2. При отсутствии задач робот будет заряжаться для поддержания минимального уровня заряда. |
| `setting_check_text` | Вид |
| `setting_consumable_change_tips1` | <br>Истек срок службы основной щетки. Требуется немедленная замена |
| `setting_consumable_change_tips2` | <br>Истек срок службы боковой щетки. Требуется немедленная замена |
| `setting_consumable_change_tips3` | <br>Истек срок службы фильтра. Требуется немедленная замена |
| `setting_consumable_change_tips4` | <br>Истек срок службы протирочной салфетки. Требуется немедленная замена |
