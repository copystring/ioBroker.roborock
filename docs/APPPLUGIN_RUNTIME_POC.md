...
Der unveränderte Q7-L5-Hermes-Pfad ist inzwischen bidirektional belegt: APK-konforme Touch- und `AndroidTextInput`-Ereignisse erreichen die originale Editier-Zustandsmaschine; nach Dialog- und Kartenbestätigung bildet das Bundle selbst die Raumumbenennungsabsicht `service.rename_room` mit `map_id`, `room_id`, `room_name` und `type_id`. Der Host zeichnet sie weiterhin nur auf.
