...
- Das unveränderte Q7-L5-Hermes-Bundle läuft nun auch mit der deterministischen
  SCMap-Vollszene durch dieselbe produktive Host-, Root- und Geräte-Ingress-
  Komposition. Das Bundle erzeugt selbst `prop.get`, `service.get_map_list`,
  `service.upload_by_maptype` und die Sprachsynchronisierung, erhält den
  entschlüsselten SDK-Blob in der von der APK belegten
  `RRDeviceBlobPayloadUpdateEvent`-Form und führt seinen originalen
  `transform`-Worker mit 96.000 Kartenwerten erfolgreich aus. Die Fixture ist
  synthetisch und datenschutzsicher; sie belegt keinen physischen Q7 und keine
  Android-Pixelparität.
