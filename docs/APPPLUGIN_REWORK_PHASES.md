...
- Ein zweiter automatisierter Fall belegt die AppPlugin-eigene Validierung eines leeren Raumnamens: Der originale Dialog bleibt mit leerem `AndroidTextInput` offen und das Bundle erzeugt keine `service.rename_room`-Absicht. Ein dritter Fall belegt die Q7-AppPlugin-eigene Längenlogik: Der Raumdialog setzt keine native `maxLength`-Prop, sondern kürzt 26 eingegebene Zeichen in seinem originalen `onChangeText` über `TEXTMAXLENGTH = 24`; `service.rename_room` enthält exakt dieses 24-Zeichen-Ergebnis.
- `npm run poc:appplugin-q7-rename-proof` baut Probe und Beweisrunner neu, prüft den unveränderten Bundle-Hash und spielt alle drei Szenarien über generische Geräte-, Pointer- und Android-TextInput-Verträge ein. Die versionierten, gerätefreien Interaktionsfixtures liegen unter `test/fixtures/appplugin/`; sanitisierte Capture-only-Ergebnisse und lokale Schlüssel bleiben in ignorierten `artifacts/`.
...
Nicht nachgewiesen sind damit automatisch andere Q7-Varianten, B01/Q10-Datenpfade, Tanos 2D/3D, die übrigen Editierabläufe, die noch offenen vordefinierten und doppelten Rename-Namen sowie Fehlerfälle oder ein sicherer Gerätebetrieb.
