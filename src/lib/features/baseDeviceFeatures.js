 ...
 */
export function RegisterModel(robotModelId) {
 ...
 */
export class BaseDeviceFeatures {
    deps;
    ...
    mapFeature(detectedFeature) {
        ...
        const potentialActionName = Feature[detectedFeature]; // Get string value (e.g., 'MopWash')
        // Find the enum key that corresponds to this string value, EXCLUDING the original detected key itself
        const mappedActionKey = Object.keys(Feature).find((key) => Feature[key] === potentialActionName && key !== detectedFeature);
        if (mappedActionKey) {
