...
const SCENARIOS: readonly ScenarioConfig[] = [
	{ scenario: "split-line-visible", fixtureSuffix: "line-visible" },
	{ scenario: "split-success", fixtureSuffix: "success" },
	{ scenario: "split-small-area", fixtureSuffix: "small-area" },
	{ scenario: "split-device-error", fixtureSuffix: "device-error" },
	{ scenario: "split-device-timeout", fixtureSuffix: "device-timeout" },
	{ scenario: "split-invalid-line", fixtureSuffix: "invalid-line" },
	{ scenario: "split-cancel", fixtureSuffix: "cancel" },
];
	...
	for (let index = 0; index < args.length; index += 1) {
		...
		if (option === "--profile") {
			...
			if (profile !== "l5" && profile !== "m5") {
				throw new Error(`Unbekanntes Q7-Split-Profil: ${profile ?? "<fehlt>"}`);
			}
		...
		throw new Error(`Unbekannte Q7-Split-Suite-Option: ${option}`);
	}
	return options;
}
...
async function runScenario(
	...
	const args = [
		...
		"--interaction", path.join(fixtureDirectory, `q7-l5-room-split-${scenario.fixtureSuffix}.json`),
...
async function main(): Promise<void> {
	...
	for (const [label, filePath] of Object.entries({
		bundle: profile.bundlePath,
		runner: path.join(options.repositoryRoot, "artifacts", "appplugin-poc", "prove_q7_appplugin_split.cjs"),
	})) {
