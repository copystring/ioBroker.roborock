		...
		return new Promise((resolve, reject) => {
			const listener102 = (deviceId, id, result) => {
				if (id == requestId) {
					...
					if (result.code) {
						reject("There was an error processing the request with id " + requestId + " error: " + JSON.stringify(result));
					}
					else {
						if (secure) {
							if (result[0] !== "ok") {
			...
			const timeout102 = this.adapter.setTimeout(() => {
				rr.off("response.102", listener102);
				this.adapter.log.error("Request with id " + requestId + " timed out after 10 seconds for response.102");
			}, 10000);
			...
			if (secure) {
				...
				timeout301 = this.adapter.setTimeout(() => {
					rr.off("response.301", listener301);
					this.adapter.log.error("Request with " + requestId + " timed out after 10 seconds for response.301");
				}, 10000);
			...
			this.adapter.messageQueue.get(requestId).timeout301 = null;
			this.checkAndClearRequest(requestId);
			this.adapter.log.error("Promise error: " + error);
		});
	}
