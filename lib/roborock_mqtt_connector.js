	...
	async sendRequest(deviceId, method, params, secure = false) {
		...
		return new Promise((resolve, reject) => {
			...
			const timeout102 = this.adapter.setTimeout(() => {
				...
				reject();
				this.adapter.log.warn("Request with id " + requestId + " timed out after 10 seconds for response.102");
			}, 10000);
			...
			if (secure) {
				...
				timeout301 = this.adapter.setTimeout(() => {
					...
					reject();
					this.adapter.log.warn("Request with " + requestId + " timed out after 10 seconds for response.301");
				}, 10000);
