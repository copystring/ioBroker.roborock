...
class roborock_mqtt_connector {
	...
	async sendRequest(deviceId, method, params, secure = false) {
		...
		const requestId = Math.floor(Math.random() * (9999 - 1000 + 1)) + 1000;
		const inner = {
			id: requestId,
			method: method,
			params: params
		...
		return new Promise((resolve, reject) => {
			const listener = (event, deviceId, id, result) => {
				if (id == requestId) {
					this.adapter.log.debug("ID debug. Requested id: " + requestId + " received id: " + id + " with event: " + event + " and this result: " + JSON.stringify(result));
