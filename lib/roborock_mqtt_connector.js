...
class roborock_mqtt_connector {
	...
	async initUser() {
		const rriot = await this.adapter.roborockApi.get_rriot();
		...
		if (this.client) {
			this.client.on("connect", (result) => {
				if (typeof result != "undefined") {
					...
					this.adapter.log.info(`MQTT connection connected ${JSON.stringify(result)}.`);
				}
			});
			...
			this.client.on("error", (result) => {
				this.adapter.catchError(
					...
					`MQTT connection error: ${result}. rriot.r.m: ${rriot.r.m} rriot.u: ${rriot.u}`,
					`client.on("error")`
				);
		...
		if (this.client) {
			...
			this.client.on("message", (topic, message) => {
				try {
					...
					// const localKeys = this.adapter.roborockApi.getMatchedLocalKeys();
