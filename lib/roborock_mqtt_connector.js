...
class roborock_mqtt_connector {
	...
	initUser(userdata, homedata) {
		...
		this.adapter.localKeys = new Map(devices.map((device) => [device.duid, device.localKey]));
	...
	async initMQTT_Subscribe() {
		...
		await client.on("close", () => {
			this.adapter.log.warn(`MQTT connection close. Trying to reconnect.`);
	...
	_decodeMsg(msg, localKey) {
		try {
			...
			this.adapter.log.error(`failed to _decodeMsg: ${error} binary data: ${JSON.stringify(mqttMessageParser.parse(msg))}`);
	...
	async initMQTT_Message() {
		...
		client.on("message", (topic, message) => {
			try {
				...
				if (localKey) {
					const data = this._decodeMsg(message, localKey);
	...
	async sendRequest(deviceId, method, params, secure = false, photo = false) {
		...
		if (!(await this.adapter.onlineChecker(deviceId))) {
			this.adapter.log.debug(`Not sending request ${method}. Device ${deviceId} is offline.`);
			return;
		}
		...
		return new Promise((resolve, reject) => {
			...
			const timeout102 = this.adapter.setTimeout(() => {
				...
				this.checkAndClearRequest(requestId);
				reject(new Error(`Request with id ${requestId} with method ${method} timed out after 10 seconds for response.102`));
			}, 10000);
	...
	sendMsgRaw(deviceId, protocol, timestamp, payload) {
		const localKey = this.adapter.localKeys.get(deviceId);
