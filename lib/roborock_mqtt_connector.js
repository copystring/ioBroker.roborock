	...
	async initMQTT_Message() {
		...
		client.on("message", (topic, message) => {
			try {
				...
				// this.adapter.log.debug(`MESSAGE RECEIVED for duid ${duid} with key: ${this.adapter.localKeys.get(duid)} data: ${JSON.stringify(data)} message: ${message}`);
				...
				if (data.protocol == 102) {
					...
					// this is for get_photo only. I think xD.
