...
class roborock_mqtt_connector {
	...
	initUser(userdata, homedata) {
		...
		setInterval(() => {
			this.adapter.log.debug("Reconnecting after 3 hours!");
			client.reconnect();
		}, 10800*1000);
	}
	...
	async initMQTT_Subscribe() {
		...
		await client.on("close", (result) => {
			this.adapter.log.error("MQTT connection close: " + result);
		});

		await client.on("reconnect", (result) => {
			this.adapter.log.error("MQTT connection reconnect: " + result);
		});
