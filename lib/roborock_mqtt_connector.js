...
class roborock_mqtt_connector {
	...
	initSharedDevices(sharedDataDevices) {
		sharedDataDevices.forEach(device => {
			localKeys.set(device.duid, device.token); // Hier wird device.token als localKey verwendet.
		});
		this.adapter.log.debug(`Shared devices test: ${JSON.stringify(Array.from(localKeys.entries()))}`);
	}
	...
	_decodeMsg(msg, localKey) {
		try {
			...
			if (version == "A01") this.adapter.log.debug(`mqttMessageParser _decodeMsg: ${JSON.stringify(data)}`);
			delete data.payloadLen;
			const aesKey = this.md5bin(this._encodeTimestamp(data.timestamp) + localKey + salt);
			if (version == "A01") this.adapter.log.debug(`aesKey _decodeMsg: ${JSON.stringify(aesKey)}`);
	...
	async initMQTT_Message() {
		...
		client.on("message", (topic, message) => {
			try {
				...
				this.adapter.log.debug(`MESSAGE RECEIVED for duid ${duid} with key: ${localKeys.get(duid)} data: ${JSON.stringify(data)} message: ${message}`);
	...
	async sendRequest(deviceId, method, params, secure = false, photo = false) {
		...
		if (!(await this.adapter.onlineChecker(deviceId))) {
			this.adapter.log.debug(`Not sending request. Device offline.`);
			return;
		}
