...
class roborock_mqtt_connector {
	...
	async initMQTT_Message() {
		...
		rr.on("foreign.message", (duid, result) => {
			...
			for (const attribute in result) {
				this.adapter.log.debug("foreign.message attribute: " + attribute + " with value: " + result[attribute]);
				switch(attribute) {
					case "121":
	...
	async sendRequest(deviceId, method, params, secure = false) {
		...
		return new Promise((resolve, reject) => {
			const listener = (event, deviceId, id, result) => {
				if (id == requestId) {
					if (event == "response.102") {

						if (secure) {
							if (result[0] != "ok") {
								this.adapter.clearTimeout(this.adapter.rrTimeout);
								reject(result);
							}
						}
						else {
					...
					else if (event == "response.301") {
						if (secure) {
							this.adapter.clearTimeout(this.adapter.rrTimeout);
							resolve(result);
						}
					}
					this.adapter.clearTimeout(this.adapter.sendRequestTimeout);
					rr.offAny(listener);
				}
				this.adapter.sendRequestTimeout = this.adapter.setTimeout(() => {
					this.adapter.log.warn("Send request timed out. Clearing event listener.");
					rr.offAny(listener);
				}, this.adapter.config.map_creation_interval*1000/2);
			};
