...
class message {
	...
	buildPayload(deviceID, messageID, method, params, secure = false, photo = false, endpoint) {
	...
	buildRoborockMessage(duid, protocol, timestamp, payload) {
		const version = this.adapter.getRobotVersion(duid);
	...
	_decodeMsg(message, duid) {
		try {
			...
			if(version == "1.0") {
				...
				return this.decryptECB(message, aesKey);
			}
			else if  (version == "A01") {
				return this.decryptECB(message, localKey);
			}
		} catch (error) {
			this.adapter.log.error(`failed to _decodeMsg: ${error} local binary data: ${message.toString("hex")}`);
	...
	decryptCBC(encrypted, localKey) {
		const iv = this.md5hex(encrypted.random.toString(16).padStart(8, "0") + "726f626f726f636b2d67a6d6da").substring(8, 24);
