...
class message {
	...
	buildRoborockMessage(deviceId, protocol, timestamp, payload) {
	...
	_decodeMsg(msg, localKey) {
		try {
			...
			const data = this.getParsedData(msg);
			...
			this.adapter.log.error(`failed to _decodeMsg: ${error} local binary data: ${msg.toString("hex")}`);
