			...
			if (client) {
				const listener102 = async (duid, id, message) => {
					if (id == messageID) {
						...
						const result = await this.adapter.message.resolve102Message(messageID, message);
						resolve(result);
					}
				};
				...
				if (secure) {
					listener301 = async (duid, id, message) => {
						...
						if (photo) {
							...
							const result = await this.adapter.message.resolve102Message(messageID, message, secure);
							resolve(result);
						}
					};
				...
				const payload = this.adapter.message.buildPayload(duid, messageID, method, params, secure, photo);
				const roborockMessage = this.adapter.message.buildRoborockMessage(duid, 4, timestamp, payload);
				...
				if (roborockMessage) {
					...
					this.adapter.catchError("Failed to build buildRoborockMessage for local connection");
				}
			}
		});
	}
	...
	async getLocalDevices() {
		return new Promise((resolve, reject) => {
			...
			server.on("message", (msg) => {
				...
				const decodedMessage = this.adapter.message.decryptECB(parsedMessage.payload, BROADCAST_TOKEN); // this might be decryptCBC for A01. Haven't checked this yet
