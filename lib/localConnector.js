		...
		client.on("data", async (message) => {
			try {
				...
				if (this.chunkBuffer.length == 0) {
					this.chunkBuffer = message;
				}
				else {
					this.chunkBuffer = Buffer.concat([this.chunkBuffer, message]);
				}

				let offset = 0;
				if (this.checkComplete(this.chunkBuffer)) {
					this.adapter.log.debug(`Data is complete. Processing...`);
					this.adapter.log.debug(`chunkBuffer: ${this.chunkBuffer.toString("hex")}`);
					while (offset + 4 <= this.chunkBuffer.length) {
						const segmentLength = this.chunkBuffer.readUInt32BE(offset);
						// length of 17 does not contain any useful data. The parser for this looks like this: const shortMessageParser = new Parser().endianess("big").string("version", {length: 3,}).uint32("seq").uint32("random").uint32("timestamp").uint16("protocol")
						if (segmentLength != 17) {
							const currentBuffer = this.chunkBuffer.subarray(offset + 4, offset + segmentLength + 4);
