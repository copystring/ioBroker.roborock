...
class MapCreator {
	...
	{
		return (image.dimensions.height/this.scaleFactor + image.position.top - robotCoord) * this.scaleFactor - 1;
	}
	...
	canvasMap(mapdata, options) {
		...
		if (mapdata.CARPET_MAP) {
			const offset = 8 * this.scaleFactor; // why the offset??
			ctx.beginPath();
			mapdata.CARPET_MAP.forEach(px => {
				...
				ctx.lineTo(this.getX(mapdata.IMAGE.dimensions, px) - offset + this.scaleFactor, (this.getY(mapdata.IMAGE.dimensions, px)) + this.scaleFactor);
			});
			ctx.stroke();
		}
