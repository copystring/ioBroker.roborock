...
class MapCreator {
	...
	async canvasMap(mapdata, selectedMap, mappedRooms, options) {
		...
		if (mapdata.IMAGE.dimensions.height > 0 && mapdata.IMAGE.dimensions.width > 0) {
			...
			this.adapter.log.info("Drawing room names on map");
			...
			Object.keys(segmentsData).forEach((segnum) => {
				...
				if (typeof selectedMap !== "undefined") {
					...
					if (mapping) {
						...
						if (!roomName) {
							this.adapter.log.warn(`Segment ${segnum} mapped to roomID ${roomID}, but no name found`);
						}
					}
					...
					if (roomName) {
						...
						this.adapter.log.info(`Drawing room "${roomName}" at (${centerX}, ${centerY})`);
						...
						// Draw transparent background for shadow
