const {
	createCanvas,
	// Canvas
} = require("canvas");

const {
	Image
} = require("canvas");

// Farben ändern
const colors = {
	floor: "#23465e",
	obstacle: "#2b2e30",
	path: "white",
	newmap: true
};
const orgcolors = [
	"#017E82",
	"#BD7B00",
	"#C05A41",
	"#4579B5",
	"#434242", // wall
	"#dfdfdf" // desel floor
];
const offset = 60;

const robot = "data:image/png;base64,";
const robot1 = "data:image/png;base64,";
const charger = "data:image/png;base64,";
const tank = "data:image/png;base64,";
const spaceship = "data:image/png;base64,";
const robot2 = "data:image/png;base64,";

//const ctximg = canvasImg.getContext('2d');

const img = new Image(); // Create a new Image
img.src = robot2;

const img_charger = new Image();
img_charger.src = charger;

const MapCreator = () => {};
MapCreator.rotateCanvas = function (img, angle) {
	const canvasImg = createCanvas(100, 100);
	const ctxImg = canvasImg.getContext("2d");
	const offset = 90;

	ctxImg.clearRect(0, 0, 100, 100);
	ctxImg.translate(100 / 2, 100 / 2);
	ctxImg.rotate((angle + offset) * Math.PI / 180);
	ctxImg.translate(-100 / 2, -100 / 2);
	ctxImg.drawImage(img, (100 / 2) - (img.width / 2), (100 / 2) - (img.height / 2));
	return canvasImg;
};

MapCreator.CanvasMap = (Mapdata, scaleFactor, options) => {

	if (options) {
		colors.floor = options.FLOORCOLOR;
		colors.obstacle = options.WALLCOLOR;
		colors.path = options.PATHCOLOR;
		colors.newmap = options && options.newmap ? options.newmap : true;
		if (options.ROBOT === "robot") {
			img.src = robot;
		} else if (options.ROBOT === "robot1") {
			img.src = robot1;
		} else if (options.ROBOT === "tank") {
			img.src = tank;
		} else if (options.ROBOT === "spaceship") {
			img.src = spaceship;
		}
	}
	let maxtop = 0;
	let maxleft = 0;
	let minleft = 0;
	let mintop = 0;

	Mapdata.image.dimensions.width = Mapdata.image.dimensions.width * scaleFactor;
	Mapdata.image.dimensions.height = Mapdata.image.dimensions.height * scaleFactor;

	function getX(px) {
		return (px * scaleFactor) % Mapdata.image.dimensions.width;
	}
	function getY(px){
		return Mapdata.image.dimensions.height - 1 - Math.floor(px * scaleFactor * scaleFactor / Mapdata.image.dimensions.width);
	}

	function robotXtoPixelX(robotCoord)
	{
		// return (robotCoord - Mapdata.image.position.left) * scaleFactor;
		return (robotCoord - Mapdata.image.position.left) * scaleFactor;
	}
	function robotYtoPixelY(robotCoord)
	{
		// return (robotCoord - Mapdata.image.dimensions.height/scaleFactor) * scaleFactor;
		return (Mapdata.image.dimensions.height/scaleFactor + Mapdata.image.position.top - robotCoord) * scaleFactor;
	}

	const canvas = createCanvas(Mapdata.image.dimensions.width, Mapdata.image.dimensions.height);
	const ctx = canvas.getContext("2d");

	if (Mapdata.image.pixels.floor && Mapdata.image.pixels.floor.length !== 0) {
		if (typeof (Mapdata.image.pixels.floor[0]) === "number" ) {
			// init min
			minleft = (Mapdata.image.pixels.floor[0] % Mapdata.image.dimensions.width);
			mintop = (Mapdata.image.dimensions.height - 1 - Math.floor(Mapdata.image.pixels.floor[0] / Mapdata.image.dimensions.width));
			["floor", "obstacle"].forEach(key => {
				ctx.beginPath();
				Mapdata.image.pixels[key].forEach(function drawPixel(px) {
					if (key === "obstacle") {
						ctx.fillStyle = colors.newmap ? orgcolors[4] : colors.obstacle;
					} else {
						ctx.fillStyle = colors.newmap ? orgcolors[5] : colors.floor;
					}
					//ctx.fillStyle = colors[key];
					ctx.rect(getX(px), getY(px), scaleFactor, scaleFactor);

					maxtop = Math.max(maxtop, getY(px));
					maxleft = Math.max(maxleft, getX(px));
					minleft = Math.min(minleft, getX(px));
					mintop = Math.min(mintop, getY(px));
					//console.log(this.maxtop, this.maxleft, this.minleft, this.mintop);
				});
				ctx.fill();
			});
		}
	}

	// Zeichne Alle Räume
	if (Mapdata.image.pixels.segments && !Mapdata.currently_cleaned_blocks && colors.newmap) {
		let segnum, lastcolor;
		ctx.beginPath();
		Mapdata.image.pixels.segments.forEach((px) => {
			segnum = (px >> 21) % 4;
			if(segnum !== lastcolor){
				ctx.fill();
				ctx.beginPath();
				ctx.fillStyle = orgcolors[segnum];
				lastcolor = segnum;
			}
			px = px & 0xfffff;
			ctx.rect(getX(px), getY(px), scaleFactor, scaleFactor);
		});
		ctx.fill();
		ctx.closePath();
	}


	if (Mapdata.currently_cleaned_blocks && colors.newmap) {
		let segnum, lastcolor;
		ctx.beginPath();
		Mapdata.image.pixels.segments.forEach(px => {
			segnum = px >> 21;
			if (Mapdata.currently_cleaned_blocks.includes(segnum)) {
				if(segnum !== lastcolor){
					ctx.fill();
					ctx.beginPath();
					ctx.fillStyle = orgcolors[segnum % 4];
					lastcolor = segnum;
				}
				px = px & 0xfffff;
				ctx.rect(getX(px), getY(px), scaleFactor, scaleFactor);
			}
		});
		ctx.fill();
		ctx.closePath();
	}


	// // Zeichne Zonen active Zonen
	// if (Mapdata.currently_cleaned_zones) {
	// 	Mapdata.currently_cleaned_zones.forEach(zone => {
	// 		const cleanMinX = Math.min(zone[0], zone[2], zone[4], zone[6]);
	// 		const cleanMinY = Math.min(zone[1], zone[3], zone[5], zone[7]);
	// 		const cleanMaxX = Math.max(zone[0], zone[2], zone[4], zone[6]);
	// 		const cleanMaxY = Math.max(zone[1], zone[3], zone[5], zone[7]);

	// 		const cleanSizeX = cleanMaxX - cleanMinX;
	// 		const cleanSizeY = cleanMaxY - cleanMinY;

	// 		ctx.fillStyle = "rgba(255, 0, 0, 0.5)";
	// 		ctx.fillRect(
	// 			robotXtoPixelX(cleanMinX/50),
	// 			robotYtoPixelY(cleanMaxY/50),
	// 			cleanSizeX/50 * scaleFactor,
	// 			cleanSizeY/50 * scaleFactor
	// 		);
	// 		ctx.lineWidth = 1 * scaleFactor / 2;
	// 		ctx.strokeStyle = "rgba(255, 0, 0, 1)";
	// 		ctx.strokeRect(
	// 			robotXtoPixelX(cleanMinX/50),
	// 			robotYtoPixelY(cleanMaxY/50),
	// 			cleanSizeX/50 * scaleFactor,
	// 			cleanSizeY/50 * scaleFactor
	// 		);
	// 	});
	// }


	// Zeichne Zonen active Zonen
	if (Mapdata.currently_cleaned_zones) {
		if (typeof Mapdata.currently_cleaned_zones[0] !== "undefined") {
			ctx.beginPath();
			Mapdata.currently_cleaned_zones.forEach(function (coord) {
				ctx.fillStyle = "rgba(46,139,87,0.1)";
				ctx.fillRect(
					robotXtoPixelX(coord[0]/50),
					robotYtoPixelY(coord[1]/50),
					robotXtoPixelX(coord[2]/50) - robotXtoPixelX(coord[0]/50),
					robotYtoPixelY(coord[3]/50) - robotYtoPixelY(coord[1]/50)
				);
				ctx.strokeStyle = "#2e8b57";
				ctx.lineWidth = 4;
				ctx.strokeRect(
					robotXtoPixelX(coord[0]/50),
					robotYtoPixelY(coord[1]/50),
					robotXtoPixelX(coord[2]/50) - robotXtoPixelX(coord[0]/50),
					robotYtoPixelY(coord[3]/50) - robotYtoPixelY(coord[1]/50)
				);
			});
		}
	}

	// Zeichne Teppich
	if (Mapdata && Mapdata.image && Mapdata.image.pixels && Mapdata.image.pixels.carpet) {
		const offset = 15 * scaleFactor; // why the offset??
		ctx.beginPath();
		Mapdata.image.pixels.carpet.forEach(px => {
			ctx.strokeStyle = "rgba(0,0,0,0.5)";
			ctx.lineWidth = 1;

			ctx.moveTo(getX(px) + offset, getY(px));
			ctx.lineTo(getX(px) + offset + 4, (getY(px)) + 4);
		});
		ctx.stroke();
	}

	// Male den Wischpfad
	if (typeof (Mapdata.path) !== "undefined" && typeof (Mapdata.mop_path) !== "undefined") {
		const mopOffset = -12; // i dont know why this offset?? Maybe the value from the end
		if (Mapdata.path.points && Mapdata.path.points.length !== 0) {
			let startX, startY; // this is needed to avoid weird spikes in sharp corners! don't remove this ever!

			ctx.beginPath();
			ctx.lineWidth = 7 * scaleFactor; // 7 makes the mop path look the same as on the Roborock app
			ctx.lineCap = "round";
			ctx.strokeStyle = "rgba(255,255,255,0.2)";

			Mapdata.path.points.forEach((coord, index) => {
				if (Mapdata.mop_path && Mapdata.mop_path[index + mopOffset] !== 0) {
					if (Mapdata.mop_path[index - 1 + mopOffset] === 0) {
						startX = robotXtoPixelX(coord[0] / 50),
						startY = robotYtoPixelY(coord[1] / 50),
						ctx.moveTo(startX, startY);
						if (Mapdata.mop_path[index + mopOffset] !== 1) {
							// see value 9 and 12 in mop_path both in front of charger
						}

					} else if (Mapdata.mop_path[index - 1 + mopOffset] !== 0) {
						ctx.moveTo(startX, startY);
						ctx.lineTo(robotXtoPixelX(coord[0] / 50), robotYtoPixelY(coord[1] / 50));
						startX = robotXtoPixelX(coord[0] / 50);
						startY = robotYtoPixelY(coord[1] / 50);
					}
				} else if (Mapdata.mop_path && Mapdata.mop_path[index + mopOffset] === 0) {
					if (Mapdata.mop_path[index - 1 + mopOffset] !== 0) {
						// do nothing ??
					}
				}

			});
			ctx.stroke();
			ctx.closePath();
		}
	}

	// Male den Pfad
	if (typeof (Mapdata.path) !== "undefined") {
		if (Mapdata.path.points && Mapdata.path.points.length !== 0) {
			ctx.fillStyle = colors.path;
			// const ctx_route = canvas.getContext("2d"); does not seem to be used anywhere
			let first = true;
			let cold1, cold2;

			ctx.beginPath();
			Mapdata.path.points.forEach((coord) => {
				if (first) {
					cold1 = robotXtoPixelX(coord[0] / 50),
					cold2 = robotYtoPixelY(coord[1] / 50),
					ctx.fillRect(cold1, cold2, 1 * scaleFactor / 2, 1 * scaleFactor / 2);
					first = false;

				} else {
					ctx.lineWidth = 1 * scaleFactor / 2;
					ctx.strokeStyle = colors.path;

					ctx.moveTo(cold1, cold2);
					cold1 = robotXtoPixelX(coord[0] / 50),
					cold2 = robotYtoPixelY(coord[1] / 50),
					ctx.lineTo(cold1, cold2);
					// ctx.stroke();
				}

			});
			ctx.stroke();
			ctx.closePath();
		}
	}

	if (Mapdata.forbidden_zones) {
		Mapdata.forbidden_zones.forEach(zone => {
			const forbiddenMinX = Math.min(zone[0], zone[2], zone[4], zone[6]);
			const forbiddenMinY = Math.min(zone[1], zone[3], zone[5], zone[7]);
			const forbiddenMaxX = Math.max(zone[0], zone[2], zone[4], zone[6]);
			const forbiddenMaxY = Math.max(zone[1], zone[3], zone[5], zone[7]);

			const forbiddenSizeX = forbiddenMaxX - forbiddenMinX;
			const forbiddenSizeY = forbiddenMaxY - forbiddenMinY;

			ctx.fillStyle = "rgba(255, 0, 0, 0.5)";
			ctx.fillRect(
				robotXtoPixelX(forbiddenMinX/50),
				robotYtoPixelY(forbiddenMaxY/50),
				forbiddenSizeX/50 * scaleFactor,
				forbiddenSizeY/50 * scaleFactor
			);
			ctx.lineWidth = 1 * scaleFactor / 2;
			ctx.strokeStyle = "rgba(255, 0, 0, 1)";
			ctx.strokeRect(
				robotXtoPixelX(forbiddenMinX/50),
				robotYtoPixelY(forbiddenMaxY/50),
				forbiddenSizeX/50 * scaleFactor,
				forbiddenSizeY/50 * scaleFactor
			);
		});
	}


	// Zeichne Ladestation wenn vorhanden
	if (Mapdata.charger) {
		if (typeof Mapdata.charger[0] !== "undefined" && typeof Mapdata.charger[1] !== "undefined") {
			ctx.beginPath();
			ctx.drawImage(img_charger,
				robotXtoPixelX(Mapdata.charger[0] / 50) - (img_charger.width / 2),
				robotYtoPixelY(Mapdata.charger[1] / 50) - (img_charger.height / 2),
				img_charger.width,
				img_charger.height
			);
		}
	}

	// Zeichne Roboter
	if (Mapdata.robot) {
		if (Mapdata.path.current_angle && typeof Mapdata.robot[0] !== "undefined" && typeof Mapdata.robot[1] !== "undefined") {
			ctx.beginPath();
			ctx.drawImage(img,
				robotXtoPixelX(Mapdata.robot[0] / 50) - (img.width / 2),
				robotYtoPixelY(Mapdata.robot[1] / 50) - (img.height / 2),
				img.width,
				img.height
			);
		} else {
			ctx.drawImage(img,
				robotXtoPixelX(Mapdata.robot[0] / 50) - (img.width / 2),
				robotYtoPixelY(Mapdata.robot[1] / 50) - (img.height / 2),
				img.width,
				img.height
			);
		}
	}

	// crop image
	const canvas_final = createCanvas(maxleft - minleft + 2 * offset, maxtop - mintop + 2 * offset);
	const ctx_final = canvas_final.getContext("2d");
	//const trimmed = ctx.getImageData(Mapdata.image.position.left, Mapdata.image.position.top, Mapdata.image.dimensions.width, Mapdata.image.dimensions.height);
	const trimmed = ctx.getImageData(minleft - offset, mintop - offset, maxleft - minleft + 2* offset, maxtop - mintop + offset);

	// canvas_final.height = maxtop - mintop + 2 * offset;
	// canvas_final.width = maxleft - minleft + 2 * offset;

	ctx_final.putImageData(trimmed, 0, 0);
	//console.log(maxtop, maxleft, minleft, mintop);

	return canvas; //.toDataURL();
};

module.exports = MapCreator;