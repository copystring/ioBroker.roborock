...
const MapCreator = () => {};
MapCreator.rotateCanvas = function (img, angleInDegrees) {
...
MapCreator.CanvasMap = (Mapdata, scaleFactor, options) => {
	...
	function robotXtoPixelX(robotCoord)
	{
		return (robotCoord - Mapdata.image.position.left) * scaleFactor;
	}
	function robotYtoPixelY(robotCoord)
	{
		return (Mapdata.image.dimensions.height/scaleFactor + Mapdata.image.position.top - robotCoord) * scaleFactor;
	}
	...
	if (Mapdata.currently_cleaned_zones) {
		if (typeof Mapdata.currently_cleaned_zones[0] !== "undefined") {
			...
			Mapdata.currently_cleaned_zones.forEach(function (coord) {
				...
				ctx.fillRect(
					robotXtoPixelX(coord[0]/50),
					robotYtoPixelY(coord[1]/50),
					robotXtoPixelX(coord[2]/50) - robotXtoPixelX(coord[0]/50),
					robotYtoPixelY(coord[3]/50) - robotYtoPixelY(coord[1]/50)
				);
			...
			Mapdata.path.points.forEach((coord, index) => {
				if (Mapdata.mop_path && Mapdata.mop_path[index + mopOffset] !== 0) {
					if (Mapdata.mop_path[index - 1 + mopOffset] === 0) {
						startX = robotXtoPixelX(coord[0] / 50);
						startY = robotYtoPixelY(coord[1] / 50);
						...
						ctx.moveTo(startX, startY);
						ctx.lineTo(robotXtoPixelX(coord[0] / 50), robotYtoPixelY(coord[1] / 50));
						startX = robotXtoPixelX(coord[0] / 50);
						startY = robotYtoPixelY(coord[1] / 50);
					}
	...
	if (typeof (Mapdata.path) !== "undefined") {
		if (Mapdata.path.points && Mapdata.path.points.length !== 0) {
			...
			Mapdata.path.points.forEach((coord) => {
				if (first) {
					cold1 = robotXtoPixelX(coord[0] / 50),
					cold2 = robotYtoPixelY(coord[1] / 50),
		...
		if (Mapdata.goto_predicted_path.points && Mapdata.goto_predicted_path.points.length !== 0) {
			...
			Mapdata.goto_predicted_path.points.forEach((coord, index) => {
				if (index === 0) {
					...
					const newCold1 = robotXtoPixelX(coord[0] / 50);
					const newCold2 = robotYtoPixelY(coord[1] / 50);
	...
	if (Mapdata.forbidden_zones) {
		Mapdata.forbidden_zones.forEach(zone => {
			...
			ctx.fillRect(
				robotXtoPixelX(forbiddenMinX/50),
				robotYtoPixelY(forbiddenMaxY/50),
				forbiddenSizeX/50 * scaleFactor,
				forbiddenSizeY/50 * scaleFactor
			);
	...
	if (Mapdata.charger) {
		if (typeof Mapdata.charger[0] !== "undefined" && typeof Mapdata.charger[1] !== "undefined") {
			ctx.beginPath();
			const img_charger_rotated = MapCreator.rotateCanvas(img_charger, Mapdata.charger.angle);
			ctx.drawImage(img_charger_rotated,
				robotXtoPixelX(Mapdata.charger[0] / 50) - (img_charger_rotated.width / 2),
				robotYtoPixelY(Mapdata.charger[1] / 50) - (img_charger_rotated.height / 2),
	...
	if (Mapdata.robot) {
		...
		if (Mapdata.path.current_angle && typeof Mapdata.robot[0] !== "undefined" && typeof Mapdata.robot[1] !== "undefined") {
			ctx.beginPath();
			const canvasImg = MapCreator.rotateCanvas(img, Mapdata.path.current_angle);
			ctx.drawImage(canvasImg,
				robotXtoPixelX(Mapdata.robot[0] / 50) - (img.width / 4),
				robotYtoPixelY(Mapdata.robot[1] / 50) - (img.height / 4),
				canvasImg.width/2,
				canvasImg.height/2
			);
		} else {
			ctx.drawImage(img,
				robotXtoPixelX(Mapdata.robot[0] / 50) - (img.width / 2),
				robotYtoPixelY(Mapdata.robot[1] / 50) - (img.height / 2),
				img.width/2,
				img.height/2
			);
		}
	...
	if (Mapdata.goto_target) {
		...
		if (typeof Mapdata.goto_target[0] !== "undefined" && typeof Mapdata.goto_target[1] !== "undefined") {
			...
			ctx.drawImage(go_to_pin_img,
				robotXtoPixelX(Mapdata.goto_target[0] / 50) - (go_to_pin_img.width / 2),
				robotYtoPixelY(Mapdata.goto_target[1] / 50) - (go_to_pin_img.height + 6),
				go_to_pin_img.width,
