	...
	canvasMap(mapdata, options) {
		...
		if (mapdata.charger) {
			if (typeof mapdata.charger[0] !== "undefined" && typeof mapdata.charger[1] !== "undefined") {
				ctx.beginPath();
				const img_charger_rotated = this.rotateCanvas(img_charger, mapdata.charger.angle);
				ctx.drawImage(img_charger_rotated,
					this.robotXtoPixelX(mapdata.image, mapdata.charger[0] / 50) - (img_charger_rotated.width / 2),
					this.robotYtoPixelY(mapdata.image, mapdata.charger[1] / 50) - (img_charger_rotated.height / 2),
		...
		if (mapdata.robot) {
			...
			if (mapdata.path.current_angle && typeof mapdata.robot[0] !== "undefined" && typeof mapdata.robot[1] !== "undefined") {
				...
				ctx.drawImage(canvasImg,
					this.robotXtoPixelX(mapdata.image, mapdata.robot[0] / 50) - (img.width / 4),
					this.robotYtoPixelY(mapdata.image, mapdata.robot[1] / 50) - (img.height / 4),
					canvasImg.width/2,
					canvasImg.height/2
				);
			} else {
		...
		if (mapdata.goto_target) {
			...
			if (typeof mapdata.goto_target[0] !== "undefined" && typeof mapdata.goto_target[1] !== "undefined") {
				...
				ctx.drawImage(go_to_pin_img,
					this.robotXtoPixelX(mapdata.goto_target[0] / 50) - (go_to_pin_img.width / 2),
					this.robotYtoPixelY(mapdata.goto_target[1] / 50) - (go_to_pin_img.height + 6),
					go_to_pin_img.width,
		...
		//const trimmed = ctx.getImageData(mapdata.image.left, mapdata.image.top, mapdata.image.dimensions.width, mapdata.image.dimensions.height);
		const trimmed = ctx.getImageData(minleft - offset, mintop - offset, maxleft - minleft + 2* offset, maxtop - mintop + offset);

		// canvas_trimmed.height = maxtop - mintop + 2 * offset;
		// canvas_trimmed.width = maxleft - minleft + 2 * offset;
