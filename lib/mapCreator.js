...
MapCreator.CanvasMap = (Mapdata, scaleFactor, options) => {
	...
	Mapdata.image.dimensions.width = Mapdata.image.dimensions.width * scaleFactor;
	Mapdata.image.dimensions.height = Mapdata.image.dimensions.height * scaleFactor;

	function getX(px) {
		return (px * scaleFactor) % Mapdata.image.dimensions.width;
	}
	function getY(px){
		return Mapdata.image.dimensions.height - 1 - Math.floor(px * scaleFactor * scaleFactor / Mapdata.image.dimensions.width);
	}
		...
		if (typeof (Mapdata.image.pixels.floor[0]) === "number" ) {
			...
			["floor", "obstacle"].forEach(key => {
				...
				Mapdata.image.pixels[key].forEach(function drawPixel(px) {
					...
					ctx.rect(getX(px), getY(px), scaleFactor, scaleFactor);

					maxtop = Math.max(maxtop, getY(px));
					maxleft = Math.max(maxleft, getX(px));
					minleft = Math.min(minleft, getX(px));
					mintop = Math.min(mintop, getY(px));
	...
	if (Mapdata.currently_cleaned_zones) {
		if (typeof Mapdata.currently_cleaned_zones[0] !== "undefined") {
			...
			Mapdata.currently_cleaned_zones.forEach(function (coord) {
				...
				ctx.fillRect((coord[0]/50 - Mapdata.image.position.left ) * scaleFactor - 12.5, (coord[1]/50 - Mapdata.image.position.top  + 102) * scaleFactor - 12.5, ((coord[2]/50 - Mapdata.image.position.top  + 102) * scaleFactor - 12.5) - ((coord[0]/50 - Mapdata.image.position.left ) * scaleFactor - 12.5), ((coord[3]/50 - Mapdata.image.position.top  + 102) * scaleFactor - 12.5) - ((coord[1]/50 - Mapdata.image.position.top  + 102) * scaleFactor - 12.5));
				ctx.strokeStyle = "#2e8b57";
				ctx.lineWidth = 4;
				ctx.strokeRect((coord[0]/50 - Mapdata.image.position.left ) * scaleFactor - 12.5, (coord[1]/50 - Mapdata.image.position.top  + 102) * scaleFactor - 12.5, ((coord[2]/50 - Mapdata.image.position.top  + 102) * scaleFactor - 12.5) - ((coord[0]/50 - Mapdata.image.position.left ) * scaleFactor - 12.5), ((coord[3]/50 - Mapdata.image.position.top  + 102) * scaleFactor - 12.5) - ((coord[1]/50 - Mapdata.image.position.top  + 102) * scaleFactor - 12.5));
			});
		}
	}
	...
	if (Mapdata && Mapdata.image && Mapdata.image.pixels && Mapdata.image.pixels.carpet) {
		const offset = 15 * scaleFactor; // why the offset??
		ctx.beginPath();
		Mapdata.image.pixels.carpet.forEach(px => {
			ctx.strokeStyle = "rgba(0,0,0,0.5)";
			ctx.lineWidth = 1;

			ctx.moveTo(getX(px) + offset, getY(px));
			ctx.lineTo(getX(px) + offset + 4, (getY(px)) + 4);
		});
		ctx.stroke();
	}
	...
	if (typeof (Mapdata.path) !== "undefined" && typeof (Mapdata.mop_path) !== "undefined") {
		...
		if (Mapdata.path.points && Mapdata.path.points.length !== 0) {
			...
			ctx.beginPath();
			ctx.lineWidth = 7 * scaleFactor; // 7 makes the mop path look the same as on the Roborock app
			ctx.lineCap = "round";
			...
			Mapdata.path.points.forEach((coord, index) => {
				if (Mapdata.mop_path && Mapdata.mop_path[index + mopOffset] !== 0) {
					if (Mapdata.mop_path[index - 1 + mopOffset] === 0) {
						startX = (coord[0]/50 - Mapdata.image.position.left ) * scaleFactor;
						startY = (coord[1]/50 - Mapdata.image.position.top  + 102) * scaleFactor + scaleFactor;
						...
						ctx.moveTo(startX, startY);
						ctx.lineTo((coord[0]/50 - Mapdata.image.position.left ) * scaleFactor, (coord[1]/50 - Mapdata.image.position.top  + 102) * scaleFactor + scaleFactor);
						startX = (coord[0]/50 - Mapdata.image.position.left ) * scaleFactor;
						startY = (coord[1]/50 - Mapdata.image.position.top  + 102) * scaleFactor + scaleFactor;
					}
	...
	if (typeof (Mapdata.path) !== "undefined") {
		if (Mapdata.path.points && Mapdata.path.points.length !== 0) {
			...
			// const ctx_route = canvas.getContext("2d"); does not seem to be used anywhere
			let first = true;
			let cold1, cold2;
			...
			Mapdata.path.points.forEach((coord) => {
				if (first) {
					cold1 = (coord[0]/50 - Mapdata.image.position.left) * scaleFactor;
					cold2 = (coord[1]/50 - Mapdata.image.position.top  + 102) * scaleFactor + scaleFactor;
					ctx.fillRect(cold1, cold2, 1 * scaleFactor / 2, 1 * scaleFactor / 2);
					first = false;
	...
	if (Mapdata.forbidden_zones) {
		Mapdata.forbidden_zones.forEach(zone => {
			...
			ctx.fillRect(
				(forbiddenMinX/50 - Mapdata.image.position.left) * scaleFactor,
				(forbiddenMinY/50 - Mapdata.image.position.top +102) * scaleFactor + scaleFactor,
				(forbiddenMaxX - forbiddenMinX)/50 * scaleFactor,
				(forbiddenMaxY - forbiddenMinY)/50 * scaleFactor
			);
			ctx.lineWidth = 1 * scaleFactor / 2;
	...
	if (Mapdata.charger) {
		if (typeof Mapdata.charger[0] !== "undefined" && typeof Mapdata.charger[1] !== "undefined") {
			ctx.beginPath();
			ctx.drawImage(img_charger, (Mapdata.charger[0]/50 - Mapdata.image.position.left ) * scaleFactor - 12.5, (Mapdata.charger[1]/50 - Mapdata.image.position.top  + 102) * scaleFactor - 12.5, img_charger.width, img_charger.height);
			// const scaledWidth = img_charger.width * scaleFactor;
			// const scaledHeight = img_charger.height * scaleFactor;
			// const scaledX = (Mapdata.charger[0]/50 - Mapdata.image.position.left) * scaleFactor - scaledWidth / 2;
			// const scaledY = (Mapdata.charger[1]/50 - Mapdata.image.position.top + 102) * scaleFactor - scaledHeight / 2;
			// ctx.drawImage(img_charger, scaledX, scaledY, scaledWidth, scaledHeight);
		}
	}
	...
	if (Mapdata.robot) {
		if (Mapdata.path.current_angle && typeof Mapdata.robot[0] !== "undefined" && typeof Mapdata.robot[1] !== "undefined") {
			ctx.beginPath();
			ctx.drawImage(img, (Mapdata.robot[0]/50 - Mapdata.image.position.left ) * scaleFactor - 12.5, (Mapdata.robot[1]/50 - Mapdata.image.position.top  + 102) * scaleFactor - 12.5, img.width, img.height);
	...
	// crop image
	const canvas_final = createCanvas(maxleft - minleft + 2 * offset, maxtop - mintop + 2 * offset);
	const ctx_final = canvas_final.getContext("2d");
	//const trimmed = ctx.getImageData(Mapdata.image.position.left, Mapdata.image.position.top, Mapdata.image.dimensions.width, Mapdata.image.dimensions.height);
