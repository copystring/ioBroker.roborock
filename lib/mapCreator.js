...
MapCreator.CanvasMap = (Mapdata, options) => {
	...
	// canvas.height = 1024; //Mapdata.image.dimensions.height;
	// canvas.width = 1024; //Mapdata.image.dimensions.width;
		...
		if (typeof (Mapdata.image.pixels.floor[0]) === "object") { //alte Valetudo version und Re bis 0.8
			...
			["floor", "obstacle"].forEach(key => {
				...
				Mapdata.image.pixels[key].forEach(function drawPixel(px) {
					...
					ctx.rect((px % Mapdata.image.dimensions.width), (Mapdata.image.dimensions.height - 1 - Math.floor(px / Mapdata.image.dimensions.width)), 1, 1);

					maxtop = Math.max(maxtop, (Mapdata.image.dimensions.height - 1 - Math.floor(px / Mapdata.image.dimensions.width)));
					maxleft = Math.max(maxleft, (px % Mapdata.image.dimensions.width));
					minleft = Math.min(minleft, (px % Mapdata.image.dimensions.width));
					mintop = Math.min(mintop, (Mapdata.image.dimensions.height - 1 - Math.floor(px / Mapdata.image.dimensions.width)));
	...
	if (Mapdata && Mapdata.image && Mapdata.image.pixels && Mapdata.image.pixels.carpet) {
		const offset = 15; // why the offset??
		ctx.beginPath();
		Mapdata.image.pixels.carpet.forEach(px => {
			ctx.strokeStyle = "rgba(0,0,0,0.5)";
			ctx.lineWidth = 1;

			ctx.moveTo((px % Mapdata.image.dimensions.width) + offset, (Mapdata.image.dimensions.height - 1 - Math.floor(px / Mapdata.image.dimensions.width)));
			ctx.lineTo((px % Mapdata.image.dimensions.width) + offset + 4, ((Mapdata.image.dimensions.height - 1 - Math.floor(px / Mapdata.image.dimensions.width))) + 4);
		});
		ctx.stroke();
	}
	...
	// crop image
	const canvas_final = createCanvas(maxtop - mintop + 2 * offset, maxleft - minleft + 2 * offset);
	...
	const returnData = [];
	returnData.push(canvas);
	returnData.push(minleft); // x start
	returnData.push(mintop); // y start
	returnData.push(canvas_final.width); // width
	returnData.push(canvas_final.height); // height

	return returnData; //.toDataURL();
};
