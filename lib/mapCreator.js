...
class MapCreator {
	...
	robotXtoPixelX(image, robotCoord) {
		return (robotCoord - image.position.left) * this.adapter.config.map_scale - 2;
	}
	robotYtoPixelY(image, robotCoord) {
		return (image.dimensions.height / this.adapter.config.map_scale + image.position.top - robotCoord) * this.adapter.config.map_scale - 2;
	}
		...
		if (mapdata.IMAGE.dimensions.height > 0 && mapdata.IMAGE.dimensions.width > 0) {
			...
			if (mapdata.IMAGE.pixels.segments && !mapdata.CURRENTLY_CLEANED_BLOCKS && colors.newmap) {
				...
				for (const { segnum } of neighborCounts) {
					if (assignedColors[segnum]) {
						this.adapter.log.debug(`Segment ${segnum} already has a color assigned: ${assignedColors[segnum]}`);
						continue;
					}
				...
				this.adapter.log.debug("Final segment colors:", assignedColors);
			}
			...
			if (mapdata.PATH && mapdata.MOP_PATH) {
				...
				if (mapdata.PATH.points && mapdata.PATH.points.length !== 0) {
					...
					ctx.beginPath();
					ctx.lineWidth = 7 * this.adapter.config.map_scale; // 7 makes the mop path look the same as on the Roborock app
					ctx.lineCap = "round";
				...
				if (typeof selectedMap !== "undefined") {
					...
					for (const mappedRoom of mappedRooms) {
						...
						if (segmentID == segnum) {
							...
							roomName = roomIDs[roomID];
							break;
						}
					}

					// If no name found, assign a placeholder name
					if (!roomName) {
						roomName = `Room ${unnamedRoomCounter++}`;
						this.adapter.log.warn(`No room name found for segment ${segnum} – assigning placeholder: ${roomName}`);
					}

					const centerX = segment.minX + (segment.maxX - segment.minX) / 2;
					const centerY = segment.minY + (segment.maxY - segment.minY) / 2;
					this.adapter.log.info(`Drawing room name "${roomName}" for segment ${segnum} at (${centerX}, ${centerY})`);
					...
					// Draw transparent square to capture background for shadow
					...
					// Draw semi-transparent rectangle with shadow
