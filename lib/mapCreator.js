...
class MapCreator {
	...
	canvasMap(mapdata, options) {
		...
		if (mapdata.charger) {
			if (typeof mapdata.charger.position[0] !== "undefined" && typeof mapdata.charger.position[1] !== "undefined") {
				ctx.beginPath();
				const img_charger_rotated = this.rotateCanvas(img_charger, mapdata.charger.angle);
				ctx.drawImage(img_charger_rotated,
					this.robotXtoPixelX(mapdata.image, mapdata.charger.position[0] / 50) - (img_charger_rotated.width / 2),
					this.robotYtoPixelY(mapdata.image, mapdata.charger.position[1] / 50) - (img_charger_rotated.height / 2),
