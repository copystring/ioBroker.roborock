	...
	PARSE(mapBuf) {
		if (mapBuf && mapBuf[0x00] === 0x72 && mapBuf[0x01] === 0x72) {
			return {
				...
				//floor: mapBuf.readUInt32LE(0x06), // don't know what this is. Most likely not the floor
				version: {
					major: mapBuf.readUInt16LE(0x08),
					minor: mapBuf.readUInt16LE(0x0a),
				},
				map_index: mapBuf.readUInt16LE(0x0C), // this might be a 32 instead of 16. Maybe check this another time
