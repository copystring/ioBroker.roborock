...
class RRMapParser {
	...
	PARSEBLOCK(buf, offset, result) {
		...
		return this.PARSEBLOCK(buf, offset + length + hlength, result);
	}
	...
	async parsedata(mapBuf) {
		...
		if (blocks[this.TYPES.IMAGE]) { //We need the image to flip everything else correctly
			...
			if (blocks[this.TYPES.CHARGER_LOCATION]) {
				parsedMapData.charger = blocks[this.TYPES.CHARGER_LOCATION].position;
				parsedMapData.charger.angle = blocks[this.TYPES.CHARGER_LOCATION].angle;
			}
