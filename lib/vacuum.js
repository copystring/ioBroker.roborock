		...
		if (this.adapter.config.enable_map_creation == true) {
			try {
				await rr_mqtt_connector.sendRequest(duid, rr, "get_map_v1", [], true).then(async map => {
					...
					this.adapter.setStateAsync("Devices." + duid + ".map.left", { val: parsedData[1], ack: true });
					this.adapter.setStateAsync("Devices." + duid + ".map.top", { val: parsedData[2], ack: true });
					...
					this.adapter.setStateAsync("Devices." + duid + ".map.mapData", { val: JSON.stringify(map.data), ack: true });
