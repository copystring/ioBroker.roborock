...
class vacuum {
		...
		if (this.adapter.config.enable_map_creation == true) {
			try {
				await rr_mqtt_connector.sendRequest(duid, rr, "get_map_v1", [], true).then(async map => {
					...
					this.adapter.log.debug("Map data: " + JSON.stringify(parsedData));
					const parsedMap = parsedData[0];
					const mapData = MapCreator.CanvasMap(parsedMap);

					this.adapter.setStateAsync("Devices." + duid + ".map.top", { val: parsedData[1], ack: true });
					this.adapter.setStateAsync("Devices." + duid + ".map.left", { val: parsedData[2], ack: true });
					this.adapter.setStateAsync("Devices." + duid + ".map.offsetY", { val: mapData[2], ack: true });
					this.adapter.setStateAsync("Devices." + duid + ".map.offsetX", { val: mapData[1], ack: true });
					this.adapter.setStateAsync("Devices." + duid + ".map.width", { val: mapData[3], ack: true });
					this.adapter.setStateAsync("Devices." + duid + ".map.height", { val: mapData[4], ack: true });

					this.adapter.setStateAsync("Devices." + duid + ".map.mapData", { val: JSON.stringify(map), ack: true });
					const mapBase64 = mapData[0].toDataURL();
	...
	async setUpObjects(duid)
	{
		...
		await this.adapter.setObjectNotExistsAsync("Devices." + duid + ".map.offsetY", {
			type: "state",
			common: {
				name: "Map offset y",
				type: "number",
		...
		await this.adapter.setObjectNotExistsAsync("Devices." + duid + ".map.offsetX", {
			type: "state",
			common: {
				name: "Map offset x",
				type: "number",
		...
		await this.adapter.setObjectNotExistsAsync("Devices." + duid + ".map.left", {
			type: "state",
			common: {
				name: "Map left coordinate",
				type: "number",
				read: true,
				write: false
			},
		...
		await this.adapter.setObjectNotExistsAsync("Devices." + duid + ".map.top", {
			type: "state",
			common: {
				name: "Map top coordinate",
				type: "number",
				read: true,
				write: false
			},
		...
		await this.adapter.setObjectNotExistsAsync("Devices." + duid + ".map.height", {
			type: "state",
			common: {
				name: "Map height",
				type: "number",
		...
		await this.adapter.setObjectNotExistsAsync("Devices." + duid + ".map.width", {
			type: "state",
			common: {
				name: "Map width",
				type: "number",
