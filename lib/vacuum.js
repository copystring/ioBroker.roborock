...
class vacuum {
	...
	async getCleaningRecordMap(duid, startTime) {
		...
		this.adapter.log.debug("start_time: " + JSON.stringify(record_map_params));

		await this.rr_mqtt_connector.sendRequest(duid, "get_clean_record_map", record_map_params, true).then(async cleaning_record_map => {

			this.mapParser.parsedata(cleaning_record_map).then(parsedData => {
				const [mapBase64, mapBase64Truncated] = this.mapCreator.canvasMap(parsedData);
			...
			this.adapter.log.warn("Failed to execute get_clean_record_map. " + error);
		});
				...
				for (const attribute in deviceStatus[0])
				{
					try {
						if (this.setup.deviceStatus[attribute]) {
							...
							this.adapter.log.error("Unsported attribute: " + attribute + " of get_status with value " + deviceStatus[0][attribute] + ". Please contact the dev to add the newly found attribute of your robot.");
						}
					} catch (e) {
					...
					case "3":
						for (const cleaningRecord in cleaningAttributes[cleaningAttribute])
						{
							...
							if (this.adapter.config.enable_map_creation == true) {
								...
								this.adapter.log.warn("Record map test:  " + JSON.stringify(mapArray["mapBase64"]));
