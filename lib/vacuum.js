		...
		try {
			switch (parameter) {
				...
				case "set_water_box_distance_off": {
					...
					const result = await this.rr_mqtt_connector.sendRequest(duid, parameter, parameterValue);
					this.adapter.log.debug(`Command: ${parameter} with value: ${JSON.stringify(parameterValue)} result: ${result}`);
					break;
				}
				case "set_homesec_password": {
					this.adapter.log.debug(`Command: ${parameter} with value: ${value}`);
					const result = await this.rr_mqtt_connector.sendRequest(duid, parameter, value);
					this.adapter.log.debug(`Command: ${parameter} with value: ${parameter} result: ${result}`);
					break;
				}
				default:
	...
	async getParameter(duid, parameter, attribute) {
		...
		try {
			if (targetFolder == "networkInfo") {
				...
				const serverTimers = await this.rr_mqtt_connector.sendRequest(duid, parameter, []);
				...
				const timers = await this.rr_mqtt_connector.sendRequest(duid, parameter, []);
				...
				const attribute_val = JSON.stringify((await this.rr_mqtt_connector.sendRequest(duid, parameter, {})));
				this.adapter.setStateAsync(`Devices.${duid}.commands.${parameter.replace("get", "set")}`, { val: attribute_val, ack: false });
				...
				this.adapter.setStateAsync(`Devices.${duid}.commands.${parameter.replace("get", "set")}`, { val: actualVal, ack: false });
