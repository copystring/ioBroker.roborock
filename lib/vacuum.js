...
class vacuum {
	...
	async changeWaterBoxCustomMode(duid, mode) {
		rr_mqtt_connector.sendRequest(duid, rr, "set_water_box_custom_mode", [mode]).then(result => {
			this.adapter.log.debug("water_box_custom_mode: " + result);
			this.getWaterBoxCustomMode(duid);
		});
	}
	...
	async getWaterBoxCustomMode(duid)
	{
		await this.adapter.setObjectNotExistsAsync("Devices." + duid + ".deviceConfig.water_box_custom_mode", {
			type: "state",
			common: {
				type: "number",
				name: "Water box custom mode",
				role: "value",
				read: true,
				write: false,
				"states": {
