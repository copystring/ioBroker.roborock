...
class vacuum {
	...
	async getParameter(duid, parameter, attribute) {
		...
		try {
			if (parameter == "get_network_info") {
				...
				if (this.adapter.socket || seconds % this.adapter.config.updateInterval == 0) { // only send status every minute or if websocket is connected
					...
					for (const attribute in deviceStatus[0]) {
						...
						if (!await this.adapter.getObjectAsync(`Devices.${duid}.deviceStatus.${attribute}`)) {
							this.adapter.catchError(`Unsported attribute: ${attribute} of get_status with value ${deviceStatus[0][attribute]}. Please contact the dev to add the newly found attribute of your robot.`);
							return;
						}
						...
						switch (attribute) {
							case "dock_type":
								...
								break;
							case "dss":
								const dockingStationStates = await this.parseDockingStationStatus(deviceStatus[0][attribute]);

								for (const state in dockingStationStates) {
									this.adapter.setStateAsync(`Devices.${duid}.dockingStationStates.${state}`, { val: dockingStationStates[state], ack: true });
								}
							case "map_status": {
	...
	async parseDockingStationStatus(dss) {
		const binaryStr = dss.toString(2).padStart(12, "0");

		return {
			cleaning_solution: binaryStr.substring(0, 2),
			water_filter: binaryStr.substring(2, 4),
			dust_container: binaryStr.substring(4, 6),
			dirty_water: binaryStr.substring(6, 8),
			clean_water: binaryStr.substring(8, 10),
			unknown: binaryStr.substring(10, 12)
		};
	}
