			...
			await this.rr_mqtt_connector.sendRequest(duid, "get_consumable", []).then(async consumables => {
				...
				for (const consumable in consumables) {
					...
					if (this.setup.consumables[consumable].divider) {
						consumable_val = consumables[consumable] / this.setup.consumables[consumable].divider;
					}
					else {
						consumable_val = consumables[consumable];
					}
					this.adapter.log.error("Setting consumable " + consumable + " with value: " + consumable_val);
			...
			for(const stream_type in this.setup.camera) {
				switch (stream_type)
				{
					case "stream_html":
						...
						break;
					case "stream_mp4":
						this.adapter.setStateAsync("Devices." + duid + ".camera." + stream_type, { val: "http://" + this.adapter.config.hostname_ip + ":1984/api/" + stream_type.replace(/_/g, ".") + "?src=" + duid + "&media=video", ack: true});
						break;
					case "rtsp":
