...
class vacuum {
		...
		if (targetFolder == "networkInfo")
		{
			...
			this.adapter.getStateAsync("Devices." + duid + ".commands.load_multi_map").then(roomFloor => {
				this.rr_mqtt_connector.sendRequest(duid, "get_room_mapping", []).then(mappedRooms => {
					...
					for (const mappedRoom in mappedRooms)
					{
						...
						this.adapter.setExistsAsync("Devices." + duid + ".floors.cleanCount", {
							type: "state",
							common: {
								name: "Clean count",
								type: "number",
			...
			this.rr_mqtt_connector.sendRequest(duid, "get_multi_maps_list", []).then(mapList => {
				...
				for (const map in mapInfo)
				{
					...
					this.adapter.setExistsAsync("Devices." + duid + ".floors." + roomFloor, {
						type: "folder",
						common: {
							name: mapName,
						},
				...
				this.adapter.setExistsAsync("Devices." + duid + ".commands.load_multi_map", {
					type: "state",
					common: {
						name: "Load map",
						type: "number",
			...
			this.rr_mqtt_connector.sendRequest(duid, parameter, []).then(async firmwareFeatures => {
				for (const firmwareFeature in firmwareFeatures) {
					...
					await this.adapter.setExistsAsync("Devices." + duid + ".firmwareFeatures." + firmwareFeature, {
						type: "state",
						common: {
							type: "string",
	...
	{
		await this.adapter.setExistsAsync("Devices." + duid, {
			type: "device",
			common: {
				statusStates: {
		...
		for (const setupAttribute in this.setup) {
			if (setupAttribute == "floor") {
				// create floors
				await this.adapter.setExistsAsync("Devices." + duid + ".floors", {
					type: "folder",
					common: {
						name: "Floors",
					},
				...
				for (const command in this.setup[setupAttribute])
				{
					...
					await this.adapter.setExistsAsync("Devices." + duid + "."+ setupAttribute + "." + command, {
						type: "state",
						common: {
							name: object.name,
							type: object.type,
				...
				for (let i=0; i<20; i++)
				{
					await this.adapter.setExistsAsync("Devices." + duid + ".cleaningInfo.Records." + i, {
						type: "folder",
						common: {
						},
						native: {},
					...
					for (const record in this.setup["cleaningRecords"]) {
						...
						await this.adapter.setExistsAsync("Devices." + duid + ".cleaningInfo.Records." + i + "." + record, {
							type: "state",
							common: {
								name: object.name,
								type: object.type,
		...
		await this.adapter.setExistsAsync("Devices." + duid + ".map.mapBase64", {
			type: "state",
			common: {
				name: "base64 Map",
				type: "string",
		...
		await this.adapter.setExistsAsync("Devices." + duid + ".map.mapBase64Truncated", {
			type: "state",
			common: {
		...
		await this.adapter.setExistsAsync("Devices." + duid + ".map.mapData", {
			type: "state",
			common: {
				name: "raw Map",
				type: "object",
			...
			for (const cleaningAttribute in cleaningAttributes)
			{
				...
				switch (cleaningAttribute) {
					...
					case "3":
						...
						await this.adapter.setExistsAsync("Devices." + duid + ".cleaningInfo.JSON", {
							type: "state",
							common: {
