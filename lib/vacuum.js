...
class vacuum {
	constructor(adapter, rrEmitter, robotModel) {
		...
		if (!supportedRobots.includes(robotModel)) {
			this.adapter.log.error("The model ${robotModel} is not supported. Get in touch with the dev to get this robot supported.");
		}

		this.adapter.log.debug("Robot key: " + Object.keys(supportedRobots).find(key => key === robotModel));

		this.extended_robot_class = require("./" + robotModel).extended_robot;
