			...
			if (parameter == "get_network_info") {
				...
				if (this.adapter.socket || seconds % this.adapter.config.updateInterval == 0) {
					...
					for (const attribute in deviceStatus[0]) {
						this.adapter.log.debug(`Starting to process attribute: ${attribute} with value: ${deviceStatus[0][attribute]}`);
						...
						if (!(await this.adapter.getObjectAsync(`Devices.${duid}.deviceStatus.${attribute}`))) {
							this.adapter.catchError(
								`Unsported attribute: ${attribute} of get_status with value ${deviceStatus[0][attribute]}. Please contact the dev to add the newly found attribute of your robot.`, "get_status", duid
							);
						...
						this.adapter.log.debug(`Processed attribute: ${attribute} with value: ${deviceStatus[0][attribute]}`);
					}
