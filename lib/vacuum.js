...
class vacuum {
				...
				await rr_mqtt_connector.sendRequest(duid, rr, "get_map_v1", [], true).then(async map => {
					...
					if (parsedData && typeof(parsedData) != "undefined") {
						if (typeof(parsedData.image) != "undefined") {
							if (typeof(parsedData.image.pixels) != "undefined") {
	...
	async command(duid, parameter, value)
	{
		if (parameter == "load_multi_map") {
			...
			this.getParameter(duid, "get_room_mapping", roomFloor);
	...
	async getParameter(duid, parameter, roomFloor) {
		...
		if (targetFolder == "networkInfo")
		{
			...
			this.adapter.getStateAsync("Devices." + duid + ".deviceStatus.map_status").then(roomFloor => {
					...
					for (const roomID in this.adapter.roomIDs)
					{
						...
						for (const mappedRoom in attribute_val)
						{
							if (roomID == attribute_val[mappedRoom][1]) {
								...
								this.adapter.setObjectNotExistsAsync("Devices." + duid + ".floors." + roomFloor.val + "." + attribute_val[mappedRoom][0], {
									type: "state",
									common: {
										name: roomName,
										type: "boolean",
