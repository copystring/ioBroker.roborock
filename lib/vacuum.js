...
class vacuum {
	...
	async command(duid, parameter, value) {
		switch (parameter) {
			case "load_multi_map":
				rr_mqtt_connector.sendRequest(duid, rr, "load_multi_map", [value]);
				break;
						...
						if (this.setup.deviceStatus[attribute]) {
							...
							if (attribute == "map_status") {
								...
								if ((deviceStatus[0]["in_cleaning"] == 0) && (deviceStatus[0]["in_returning"] == 0)) {
									rr_mqtt_connector.sendRequest(duid, rr, "load_multi_map", [deviceStatus[0][attribute]]).then(() => {
								...
								// 	if (currentMap?.val != deviceStatus[0][attribute]) {
								// 		this.adapter.setStateAsync("Devices." + duid + ".commands.load_multi_map", { val: deviceStatus[0][attribute], ack: true });
								// 	}
								// });
							}
