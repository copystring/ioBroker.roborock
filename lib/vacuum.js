...
const rrLocalConnector = require("./localConnector").localConnector;
...
class vacuum {
	constructor(adapter, robotModel) {
		...
		this.localConnector = new rrLocalConnector(this.adapter);
	...
	async getMap(duid) {
		if (this.adapter.config.enable_map_creation) {
			...
			try {
				// const map = await connector.sendRequest(duid, this.adapter.idCounter++, "get_map_v1", [], true);
				const map = await this.rr_mqtt_connector.sendRequest(duid, this.adapter.idCounter++, "get_map_v1", {}, true);
				const mappedRooms = await connector.sendRequest(duid, this.adapter.idCounter++, "get_room_mapping", []);

				const deviceStatus = await connector.sendRequest(duid, this.adapter.idCounter++, "get_status", []);
	...
	async getCleaningRecordMap(duid, startTime) {
		try {
			const cleaningRecordMap = await this.rr_mqtt_connector.sendRequest(duid, this.adapter.idCounter++, "get_clean_record_map", { start_time: startTime }, true);
	...
	async command(duid, parameter, value) {
		...
		try {
			switch (parameter) {
				case "load_multi_map": {
					const result = await connector.sendRequest(duid, this.adapter.idCounter++, "load_multi_map", value);
				...
				case "app_segment_clean": {
					...
					const mappedRoomList = await connector.sendRequest(duid, this.adapter.idCounter++, "get_room_mapping", []);
					...
					const result = await connector.sendRequest(duid, this.adapter.idCounter++, "app_segment_clean", [roomList]);
				...
				case "reset_consumable":
					await connector.sendRequest(duid, this.adapter.idCounter++, parameter, [value]);
					this.adapter.log.info(`Consumable ${parameter} successfully reset.`);

					break;

				case "app_set_dryer_status": {
					const result = await connector.sendRequest(duid, this.adapter.idCounter++, parameter);
				...
				case "app_zoned_clean": {
					const result = await connector.sendRequest(duid, this.adapter.idCounter++, parameter, value);
				...
				case "set_water_box_distance_off": {
					...
					const result = await connector.sendRequest(duid, this.adapter.idCounter++, parameter, parameterValue);
				...
				default:
					if (value && typeof value !== "boolean") {
						...
						const result = await connector.sendRequest(duid, this.adapter.idCounter++, this.adapter.idCounter++, parameter, value);
						this.adapter.log.debug(`Command: ${parameter} with value: ${JSON.stringify(value)} result: ${result}`);
	...
	async getParameter(duid, parameter, attribute) {
		...
		try {
			if (targetFolder == "networkInfo") {
				mode = parameter;
				const networkInfo = await connector.sendRequest(duid, this.adapter.idCounter++, parameter, []);
				...
				const consumables = (await connector.sendRequest(duid, this.adapter.idCounter++, "get_consumable", []))[0];
				...
				const mapList = await connector.sendRequest(duid, this.adapter.idCounter++, "get_multi_maps_list", []);
				...
				const firmwareFeatures = await connector.sendRequest(duid, this.adapter.idCounter++, parameter, []);
				...
				const serverTimers = await connector.sendRequest(duid, this.adapter.idCounter++, parameter, []);
				...
				const timers = await connector.sendRequest(duid, this.adapter.idCounter++, parameter, []);
				...
				const attribute_val = JSON.stringify(await connector.sendRequest(duid, this.adapter.idCounter++, parameter, {}));
				...
				const attribute_val = await connector.sendRequest(duid, this.adapter.idCounter++, parameter, {});
				...
				// unknown parameter
				const unknown_parameter_val = await connector.sendRequest(duid, this.adapter.idCounter++, parameter, []);
	...
	setUpObjects(duid) {
		this.adapter.setObjectAsync("Devices." + duid, {
			type: "device",
			common: {
			...
			if (setupAttribute != "firmwareFeatures") {
				for (const command in this.setup[setupAttribute]) {
					...
					if (setupAttribute != "cleaningRecords") {
						...
						this.adapter.createStateObjectHelper(objectString, object.name, object.type, object.unit, object.def, "value", true, object.write, object.states);
					} else {
						for (let i = 0; i < 20; i++) {
							this.adapter.setObjectAsync(`Devices.${duid}.cleaningInfo.Records.${i}`, {
								type: "folder",
								common: {
							...
							for (const record in this.setup["cleaningRecords"]) {
								...
								this.adapter.createStateObjectHelper(objectString, object.name, object.type, object.unit, null, "value", true, object.write);

								this.adapter.setObjectAsync(`Devices.${duid}.cleaningInfo.Records.${i}.map`, {
									type: "folder",
									common: {
										name: "Map",
									},
								...
								for (const name of ["mapBase64", "mapBase64Truncated", "mapData"]) {
									...
									this.adapter.createStateObjectHelper(objectString, name, "string", null, null, "value", true, false);
								}
							}
						}
					}
	...
	async getCleanSummary(duid) {
		...
		try {
			const cleaningAttributes = await connector.sendRequest(duid, this.adapter.idCounter++, "get_clean_summary", []);
			...
			for (const cleaningAttribute in cleaningAttributes) {
				...
				if (["clean_time", "clean_area", "clean_count"].includes(mappedAttribute)) {
					...
					for (const cleaningRecord in cleaningAttributes[cleaningAttribute]) {
						...
						const cleaningRecordAttributes = (await connector.sendRequest(duid, this.adapter.idCounter++, "get_clean_record", [cleaningRecordID]))[0];
