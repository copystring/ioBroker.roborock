...
class vacuum {
	...
	async deviceInfo(duid, device) {
		for (const attribute in device) {
			...
			if (type == "object" && attribute_val != null) {
				for (const attribute_in_object in device[attribute]) {
					...
					if ((attribute_in_object == "fan_power") || (attribute_in_object == "water_box_mode")) {
						...
						this.adapter.log.debug("deviceStatus " + attribute_in_object + " set with value: " + parseInt(attribute_val_in_object));
			...
			else if ((attribute == "activeTime") || (attribute == "shareTime")){
				await this.adapter.setStateAsync("Devices." + duid + ".deviceInfo." + attribute, { val: parseInt(attribute_val/60/60/10000), ack: true });
			}
			else {
