...
class vacuum {
	...
	async getMap(duid) {
		if (this.adapter.config.enable_map_creation) {
			...
			try {
				...
				try {
					fs.writeFileSync("./test.rrmap", map);
				}
				catch (error) {
					this.adapter.catchError(error, "writing test.rrmap", duid);
				}
	...
	async getCleaningRecordMap(duid, startTime) {
		...
		try {
			const cleaningRecordMap = await connector.sendRequest(duid, this.adapter.idCounter++, "get_clean_record_map", { start_time: startTime }, true);
	...
	async getParameter(duid, parameter, attribute) {
		...
		try {
			if (targetFolder == "networkInfo") {
					...
					switch (attribute) {
						case "map_status":
							...
							if (this.adapter.isCleaning(deviceStatus[0]["state"])) {
								...
								if (maxMaps) { // sometimes maxMaps is not available shortly after first start of adapter
									if (maxMaps.val > 1) {
				...
				const photoresponse = await connector.sendRequest(
					duid,
					"get_photo",
					{
						data_filter: {
			...
			else if (parameter == "get_dust_collection_switch_status" ||
