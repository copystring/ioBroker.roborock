...
class vacuum {
	...
	async getCleaningRecordMap(duid, startTime) {
		try {
			...
			return {
				mapBase64: parsedData.mapBase64,
				mapBase64Truncated: parsedData.mapBase64Truncated,
	...
	async getParameter(duid, parameter, attribute) {
		...
		try {
			if (targetFolder == "networkInfo") {
				...
				const consumables = await this.rr_mqtt_connector[0].sendRequest(duid, "get_consumable", []);
				...
				for (const mappedRoom of mappedRooms) {
					const roomID = mappedRooms[mappedRoom][1];
					const roomName = this.adapter.roomIDs[roomID];

					if (roomName) {
						this.adapter.log.debug(`Mapped room matched: ${roomID} with name: ${roomName}`);
						const objectString = `Devices.${duid}.floors.${roomFloor}.${mappedRooms[mappedRoom][0]}`;
						this.adapter.createStateObjectHelper(objectString, roomName, "boolean", true, "value", true, true);
					}
				}

				const objectString = `Devices.${duid}.floors.cleanCount`;
				this.adapter.createStateObjectHelper(objectString, "Clean count", "number", 1, "value", true, true);
				...
				if (mapList[0]["max_multi_map"] > 1) {
					this.adapter.createStateObjectHelper(commandPath, "Load map", "number", "value", 0, true, true, maps);
				} else {
				...
				for (const firmwareFeature in firmwareFeatures) {
					...
					await this.adapter.createStateObjectHelper(objectString, "string", featureID.toString(), "value", true, false);
			...
			this.adapter.log.error(`Error in getParameter(): ${error}`);

			if (this.adapter.supportsFeature && this.adapter.supportsFeature("PLUGINS")) {
				if (this.adapter.sentryInstance) {
					this.adapter.sentryInstance.getSentryObject().captureException(`get_status error: ${error}`);
				}
			}
		}
	}
			...
			if (setupAttribute != "cleaningRecords" && setupAttribute != "firmwareFeatures") {
				...
				for (let i = 0; i < 20; i++) {
					...
					for (const record in this.setup["cleaningRecords"]) {
						...
						await this.adapter.createStateObjectHelper(objectString, object.name, object.type, object.unit, "value", true, object.write);
						...
						for (const mapType of ["mapBase64", "mapBase64Truncated", "mapData"]) {
							const objectString = "Devices." + duid + ".cleaningInfo.Records." + i + ".map." + mapType;
							await this.adapter.createStateObjectHelper(objectString, mapType, "string", "value", true, false);
						}
					}
				}
			}
				...
				for (const cleaningAttribute in cleaningAttributes) {
					...
					switch (cleaningAttribute) {
						...
						case "3": {
							...
							await this.adapter.createStateObjectHelper(objectString, "cleaningInfoJSON", "string", "json", true, false);
