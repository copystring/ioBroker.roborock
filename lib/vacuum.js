...
class vacuum {
				...
				await rr_mqtt_connector.sendRequest(duid, rr, "get_map_v1", [], true).then(async map => {
					await this.mapParser.parsedata(map).then(async parsedData => {
						...
						if (typeof(parsedData?.image?.pixels?.floor) != "undefined") {
							const mapBase64 = this.mapCreator.canvasMap(parsedData).toDataURL();
							this.adapter.setStateAsync("Devices." + duid + ".map.mapBase64", { val: mapBase64, ack: true });

							mapImage.src = mapBase64;
							const tempCanvas = createCanvas(mapImage.width, mapImage.height);
							const tempCtx = tempCanvas.getContext("2d");
							let mapMinX = mapImage.width, mapMinY = mapImage.height, mapMaxX = 0, mapMaxY = 0;
							tempCtx.drawImage(mapImage, 0, 0);
							const imageData = tempCtx.getImageData(0, 0, mapImage.width, mapImage.height);
							...
							truncatedCtx.drawImage(mapImage, mapMinX, mapMinY, mapSizeX, mapSizeY, 0, 0, mapSizeX, mapSizeY);
	...
	async command(duid, parameter, value) {
		switch (parameter) {
			case "load_multi_map":
				rr_mqtt_connector.sendRequest(duid, rr, "load_multi_map", value).then(async result => {
					if (result[0] == "ok") {
			...
			case "app_segment_clean":
				...
				await this.adapter.getStateAsync("Devices." + duid + ".commands.load_multi_map").then(async roomFloor => {
					await rr_mqtt_connector.sendRequest(duid, rr, "get_room_mapping", []).then(async mappedRoomList => {
						...
						await this.adapter.getStateAsync("Devices." + duid + ".floors.cleanCount").then(cleanCount => {
							...
							rr_mqtt_connector.sendRequest(duid, rr, parameter, [roomList]).then(result => {
	...
	async getParameter(duid, parameter) {
		...
		{
			mode = parameter;
			rr_mqtt_connector.sendRequest(duid, rr, parameter, []).then(networkInfo => {
			...
			rr_mqtt_connector.sendRequest(duid, rr, "get_status", []).then(deviceStatus => {
			...
			rr_mqtt_connector.sendRequest(duid, rr, "get_multi_maps_list", []).then(mapList => {
		...
		else if (typeof(this.parameterFolders[parameter]) != "undefined")
		{
			...
			// unknown parameter
			rr_mqtt_connector.sendRequest(duid, rr, parameter, []).then(unknown_parameter_val => {
				// this.adapter.setStateAsync("Devices." + duid + "." + targetFolder + "." + mode, { val: attribute_val[0], ack: true });
