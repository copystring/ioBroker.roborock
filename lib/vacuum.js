...
class vacuum {
	...
	async getMap(duid) {
		if (this.adapter.config.enable_map_creation) {
			...
			try {
				...
				const mappedRooms = await connector.sendRequest(duid, "get_room_mapping", []);

				const deviceStatus = await connector.sendRequest(duid, "get_status", []);
	...
	async getCleaningRecordMap(duid, startTime) {
		try {
			const cleaningRecordMap = await this.adapter.rr_mqtt_connector.sendRequest(duid, "get_clean_record_map", { start_time: startTime }, true);
	...
	async command(duid, parameter, value) {
		...
		try {
			switch (parameter) {
				case "load_multi_map": {
					const result = await connector.sendRequest(duid, "load_multi_map", value);
				...
				case "app_segment_clean": {
					...
					const mappedRoomList = await connector.sendRequest(duid, "get_room_mapping", []);
					...
					const result = await connector.sendRequest(duid, "app_segment_clean", [roomList]);
				...
				case "reset_consumable":
					await connector.sendRequest(duid, parameter, [value]);
					this.adapter.log.info(`Consumable ${parameter} successfully reset.`);

					break;

				case "app_set_dryer_status": {
					const result = await connector.sendRequest(duid, parameter, JSON.parse(value));
				...
				case "app_zoned_clean": {
					const result = await connector.sendRequest(duid, parameter, value);
				...
				case "set_water_box_distance_off": {
					...
					const result = await connector.sendRequest(duid, parameter, parameterValue);
				...
				default:
					if (value && typeof value !== "boolean") {
						...
						const result = await connector.sendRequest(duid, parameter);
						this.adapter.log.debug(`Command: ${parameter} result: ${result}`);
					}
			}
		} catch (error) {
	...
	async getParameter(duid, parameter, attribute) {
		...
		try {
			if (targetFolder == "networkInfo") {
				mode = parameter;
				const networkInfo = await connector.sendRequest(duid, parameter, []);
				...
				const consumables = (await connector.sendRequest(duid, "get_consumable", []))[0];
				...
				const mapList = await connector.sendRequest(duid, "get_multi_maps_list", []);
				...
				const firmwareFeatures = await connector.sendRequest(duid, parameter, []);
				for (const firmwareFeature in firmwareFeatures) {
					...
					this.adapter.setStateAsync(objectString, { val: this.adapter.translations[`firmwareFeatures_${firmwareFeature}`], ack: true });
				}
			} else if (parameter == "get_server_timer") {
				const serverTimers = await connector.sendRequest(duid, parameter, []);
				...
				const timers = await connector.sendRequest(duid, parameter, []);
				...
				const attribute_val = JSON.stringify(await connector.sendRequest(duid, parameter, {}));
				...
				const attribute_val = await connector.sendRequest(duid, parameter, {});
				...
				// unknown parameter
				const unknown_parameter_val = await connector.sendRequest(duid, parameter, []);
	...
	async getCleanSummary(duid) {
		...
		try {
			const cleaningAttributes = await connector.sendRequest(duid, "get_clean_summary", []);
			...
			for (const cleaningAttribute in cleaningAttributes) {
				...
				if (["clean_time", "clean_area", "clean_count"].includes(mappedAttribute)) {
					...
					for (const cleaningRecord in cleaningAttributes[cleaningAttribute]) {
						...
						const cleaningRecordAttributes = (await connector.sendRequest(duid, "get_clean_record", [cleaningRecordID]))[0];
