...
class vacuum {
	...
	async getMap(duid) {
		if (this.adapter.config.enable_map_creation) {
			...
			try {
				...
				const map = await this.adapter.rr_mqtt_connector.sendRequest(duid, this.adapter.idCounter++, "get_map_v1", {}, true);
	...
	async getCleaningRecordMap(duid, startTime) {
		try {
			const cleaningRecordMap = await this.adapter.rr_mqtt_connector.sendRequest(duid, this.adapter.idCounter++, "get_clean_record_map", { start_time: startTime }, true);
