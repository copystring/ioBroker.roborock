...
class vacuum {
	constructor(adapter, rrEmitter) {
		...
		this.states = {
			...
			// "water_box_custom_mode": {"200": "Off","201": "Low","202": "Medium","203": "High","204": "Customize (Auto)","207": "Custom (Levels)"},
			// "water_box_mode": {"201": "Mild","202": "Moderate","203": "Intense"},
			"water_box_mode": {"200": "Off","201": "Low","202": "Medium","203": "High","204": "Customize (Auto)","207": "Custom (Levels)"},
			"water_box_custom_mode": {"201": "Mild","202": "Moderate","203": "Intense"},
			"fan_power": {"101": "Quiet", "102": "Balanced", "103": "Turbo", "104": "Max"}
		};
		...
		this.setup = {
			...
			["commands"]: {
				...
				"set_water_box_mode":           {"type": "number", "name":"I don't know what this does yet. I guess my robot does not support this.", "def":200, "states": this.states["water_box_mode"], "write": true},
			...
			["deviceConfig"]: {
				...
				"water_box_mode":               {"type": "number", "name":"I don't know what this does yet. I guess my robot does not support this.", "states": this.states["water_box_mode"], "write": false},
	...
	async command(duid, parameter, value)
	{
		if (parameter == "load_multi_map") {
			...
			await rr_mqtt_connector.sendRequest(duid, rr, parameter, roomList).then(result => {
				this.adapter.log.debug("app_segment_clean with roomIDs: " + JSON.stringify(roomList));
			});
		}
	...
	async getParameter(duid, parameter) {
		...
		if (targetFolder == "networkInfo")
		{
		...
		else if ((parameter == "get_custom_mode") || (parameter == "get_status")) {
			rr_mqtt_connector.sendRequest(duid, rr, "get_status", []).then(attribute_val => {
				for (const attribute in attribute_val[0])
				{
					...
					// else {
					// 	this.adapter.setStateAsync("Devices." + duid + ".deviceInfo." + attribute, { val: attribute_val[0][attribute], ack: true });
					// }
				}
			});
		}
