		...
		if (this.adapter.config.enable_map_creation == true) {
			try {
				await rr_mqtt_connector.sendRequest(duid, rr, "get_map_v1", [], true).then(async map => {
					...
					this.adapter.setStateAsync("Devices." + duid + ".map.mapData", { val: parsedData, ack: true });
					if (typeof(parsedData) != "undefined") {
						if (typeof(parsedData[0].image) != "undefined") {
							if (typeof(parsedData[0].image.pixels) != "undefined") {
								if (typeof(parsedData[0].image.pixels.floor) != "undefined") {
									const mapBase64 = MapCreator.CanvasMap(parsedData[0], this.adapter.config.map_scale).toDataURL();
									this.adapter.setStateAsync("Devices." + duid + ".map.mapBase64", { val: mapBase64, ack: true });

									const image = new Image();
									image.src = mapBase64;
									this.adapter.log.debug("image loaded!");
									...
									{
										this.adapter.socket.send(mapToSend);
									}
