		...
		if (this.adapter.config.enable_map_creation == true) {
			try {
				await rr_mqtt_connector.sendRequest(duid, rr, "get_map_v1", [], true).then(async map => {
					...
					this.adapter.setStateAsync("Devices." + duid + ".map.width", { val: parsedData[3], ack: true });
					this.adapter.setStateAsync("Devices." + duid + ".map.height", { val: parsedData[4], ack: true });

					this.adapter.log.debug("Top: " + parsedData[1] + " left: " + parsedData[2] + " width: " + parsedData[3] + " height: " + parsedData[4]);
					...
					const test = mapData[1];
					this.adapter.log.debug("mapTest: " + test);
