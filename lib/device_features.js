...
class device_features {
	...
	isShakeMopSetSupported() {
		// this.isSupportedWaterMode(); // can not use this for isSupportedWaterMode() and most likely not for anything else. See https://github.com/copystring/ioBroker.roborock/issues/760
		// nothing for now
	}
	...
	isAvoidCarpetSupported(duid) {
		const robotModel = this.adapter.http_api.getRobotModel(duid);

		this.adapter.log.debug(`isAvoidCarpetSupported ${robotModel}`);
		...
		switch (robotModel) {
			case "roborock.vacuum.a72":
				// special case. don't add carpet_clean_mode
				break;
			default:
