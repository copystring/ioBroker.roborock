	...
	async getHomeID() {
		...
		if (this.cachedHomeID) {
			...
			this.loginApi
				...
				.then(res => async () => {
					if (res.data.data) {
						await this.adapter.setState("cachedHomeID", { val: res.data.data.rrHomeId, ack: true });
						return res.data.data.rrHomeId;
					}
					else {
						const cachedHomeID = this.adapter.getState("cachedHomeID");
						if (cachedHomeID) {
							this.cachedHomeID = cachedHomeID.val;
							return cachedHomeID.val;
						}
						else {
							this.adapter.log.error(`failed to get getHomeDetail: ${res.data.msg}`);
							return null;
						}
	...
	async updateHomeData() {
		...
		try {
			...
			if (homeId) {
				...
				this.homeData = await this.realApi.get(`v2/user/homes/${homeId}`).then((res) => res.data.result);
