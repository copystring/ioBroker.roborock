...
const actions = {
	...
	set_cleaned_area: () => {
		cleaningRecordsString.manual_replenish = { type: "number", name: "cleaning_record_cleaned_area", unit: "m²" };
	},
	set_clean_times: () => {
		cleaningRecordsString.manual_replenish = { type: "number", name: "cleaning_record_clean_times" };
	},
	set_switch_status: () => {
	...
	async processSupportedFeatures() {
		if (this.productCategory == "robot.vacuum.cleaner") {
			...
			const modelConfig = {
				...
				// S6 MaxV
				"roborock.vacuum.a10": ["setCleaningRecordsString", "setConsumablesInt"],
				// S7
