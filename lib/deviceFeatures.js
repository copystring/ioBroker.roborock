...
const actions = {
	...
	set_CleaningRecordsS6MaxV: async (context) => { // this is a special case for the S6 MaxV only as far as I know
		context.cleaningRecords = cleaningRecordsString;
		context.cleaningInfo = cleaningInfoInt;
	},
...
class deviceFeatures {
	...
	async processSupportedFeatures() {
		if (this.productCategory == "robot.vacuum.cleaner") {
			...
			const modelConfig = {
				// S6 Pure
				"roborock.vacuum.a08": ["setCleaningRecordsString", "setConsumablesInt"],
				// S6 MaxV
				"roborock.vacuum.a10": ["set_CleaningRecordsS6MaxV", "setConsumablesInt"],
				// S7
				...
				// S5 Max
				"roborock.vacuum.s5e": ["setCleaningRecordsString", "setConsumablesInt"],
				// S6
			...
			for (const [, state] of Object.entries(dockingStationStates)) {
				await this.adapter.createDockingStationObject(this.duid, state);
			}
