...
const resetConsumables = [ "main_brush_work_time", "side_brush_work_time", "filter_work_time", "filter_element_work_time", "sensor_dirty_time", "dust_collection_work_times", "filter_element_work_time"];
...
class deviceFeatures {
	...
	// this.adapter.createDeviceStatus(this.duid, "mop_forbidden_enable", "number");
	// }
	...
	isCustomWaterBoxDistanceSupported() {
		...
		this.adapter.setObjectAsyn(`Devices.${this.duid}.commands.set_water_box_distance_off`, {
			type: "state",
			common: {
	...
	async processSupportedFeatures() {
		if (this.productCategory == "robot.vacuum.cleaner") {
			...
			const oldStyle = (
				this.model == "roborock.vacuum.s4" || // S4
				this.model == "roborock.vacuum.a19" || // S4 Max
				this.model == "roborock.vacuum.s5e" || // S5 Max
				this.model == "roborock.vacuum.s6" || // S6
				this.model == "roborock.vacuum.a08" || // S6 Pure
				this.model == "roborock.vacuum.a10" // S6 MaxV
			);

			this.cleaningRecords = oldStyle ? cleaningRecordsInt: cleaningRecordsString;
			...
			for (const [consumable, object] of Object.entries(consumables)) {
			...
			if (
				this.model == "roborock.vacuum.a38" || // Q7 Max
				this.model == "roborock.vacuum.a72" || // Q5 Pro
				this.model == "roborock.vacuum.a73" || // Q8 Max
				this.model == "roborock.vacuum.a75" || // Q Revo
				this.model == "roborock.vacuum.a70" || // S8 Pro Ultra
				this.model == "roborock.vacuum.a51" // S8
			) {
				this.cleaningRecords.map_flag = { type: "number", name: "cleaning_record_map_flag" };
			}
			if (
				this.model == "roborock.vacuum.a73" // Q8 Max
			) {
			...
			// process cleaningInfo
			const cleaningInfo = oldStyle ? cleaningInfoInt: cleaningInfoString;
			for (const [cleaningInfoKey, cleaningInfoEntry] of Object.entries(cleaningInfo)) {
				this.adapter.log.debug(`cleaningInfoEntry ${cleaningInfoEntry}`);
		...
		else if (this.productCategory == "roborock.vacuum") { // vacuum (not sure if it's actually roborock.vacuum. Might be something else. Haven't testet)
			this.adapter.createBasicVacuumObjects(this.duid);
		}
		else if (this.productCategory == "roborock.wm") { // washing machine
