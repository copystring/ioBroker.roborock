...
class deviceFeatures {
	...
	isCameraSupported() {
		...
		for (const stream_type in streamTypes) {
			this.adapter.setObjectAsync(`Devices.${this.duid}.camera.${stream_type}`, {
				type: "state",
				common: {
					name: stream_type,
					type: "number",
	...
	getFeatureList() {
		return {
			...
			isMopForbiddenSupported: ["roborock.vacuum.a51"].includes(this.model),
	...
	async processSupportedFeatures() {
		if (this.productCategory == "robot.vacuum.cleaner") {
			...
			// process consumables
			this.adapter.log.debug(`Consumables: ${JSON.stringify(this.consumables)}`);
