...
const actions = {
	...
	setCleaningRecordsMixed: async (context) => { // this is a special case some olders robots
		context.cleaningRecords = cleaningRecordsString;
		context.cleaningInfo = cleaningInfoInt;
	},
	...
	async processSupportedFeatures() {
		if (this.productCategory == "robot.vacuum.cleaner") {
			...
			const modelConfig = {
				...
				// S7 MaxV (Ultra)
				"roborock.vacuum.a27": ["setCleaningRecordsString", "setConsumablesInt", "set_map_flag", "set_back_type", "set_charge_status", "set_clean_percent", "set_switch_status"],
				// Q7 Max
