...
class local_api {
	...
	decryptECB(encrypted, aesKey) {
		...
		try {
			let decrypted = Buffer.concat([decipher.update(input), decipher.final()]);
			decrypted = this.removePadding(decrypted);
			return decrypted;
		} catch (e) {
			this.adapter.log.error(`[decryptECB] Failed to decrypt! Error: ${e.message} encrypted (hex): ${input.toString("hex")} length: ${input.length} key: ${aesKey.toString("hex")}`);
			throw e;
		}
	}
