	...
	async getLocalDevices() {
		...
		return new Promise((resolve, reject) => {
			...
			const handleMessage = async (msg) => {
				this.adapter.log.debug(`getLocalDevices() package received: ${msg.toString("hex")}`);
				try {
					const parsedMessage = localMessageParser.parse(msg);
					const decodedMessage = this.decryptECB(parsedMessage.payload);
	...
	decryptECB(encrypted) {
		...
		try {
			...
			this.adapter.log.error(
				`[decryptECB] Failed to decrypt! Error: ${e.message} encrypted (hex): ${input.toString("hex")} length: ${input.length}}`
			);
			throw e;
		}
	}
