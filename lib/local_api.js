		...
		return new Promise((resolve, reject) => {
			...
			const handleMessage = async (msg) => {
				...
				if (version === "L01") {
					...
					this.adapter.warn(`Unknown protocol version "${version}" found in local discovery packet. Please report this! Raw: ${msg.toString("hex")}`);
					return;
				}
				...
				try {
					...
					this.adapter.warn(`Failed to process message: ${error.stack}`);
				}
			};
