...
class local_api {
	...
	async createClient(duid, ip) {
		...
		client.on("close", () => {
			this.adapter.log.debug(`TCP client for ${duid} disconnected, attempting reconnect via initiateClient...`);
			setTimeout(() => this.initiateClient(duid), 60000);
			client.connected = false;
		});

		client.on("error", (error) => {
			this.adapter.log.debug(`TCP error for ${duid}: ${error.message}`);
		});
	}
