...
window.onload = function() {
	...
	(async () => {
		...
		function getMouseX(x) {
			return Math.floor((x - canvasOffsetX - 2 - panOffsetX) / zoomLevel / wheelZoom);
		}
		function getMouseY(y) {
			return Math.floor((y - canvasOffsetY - 2 - panOffsetY) / zoomLevel / wheelZoom);
		}

		function getOriginalX(transformedX) {
			return Math.floor(((transformedX - data.map.IMAGE.position.left) * scaleFactor - 2) - mapMinX);
		}
		function getOriginalY(transformedY) {
			return Math.floor(((data.map.IMAGE.dimensions.height / scaleFactor + data.map.IMAGE.position.top - transformedY) * scaleFactor - 2) - mapMinY);
		}
		...
		async function downEvent(e) {
			...
			for (let i = 0; i < rects.length; i++) {
				...
				const distanceRedDot = Math.sqrt(Math.pow(mouseX - (rect.x + rect.width), 2) + Math.pow(mouseY - (rect.y + rect.height), 2));
			...
			if (data.map.OBSTACLES2) {
				const obstacleX = getOriginalX(data.map.OBSTACLES2[1][0] / 50);
				const obstacleY = getOriginalY(data.map.OBSTACLES2[1][1] / 50);
				console.log(`Obstacle test. obstacleX: ${obstacleX} obstacleY: ${obstacleY}`);

				data.map.OBSTACLES2.forEach((obstacle) => {
					const x = getOriginalX(obstacle[0] / 50);
					const y = getOriginalY(obstacle[1] / 50);
		...
		addButton.addEventListener("click", function() {
			...
			const x = 650;
			const y = 460;
			const width = 25 * scaleFactor;
			const height = 25 * scaleFactor;
		...
		function updatePopupPosition() {
			if (popupTimeout) {
				...
				popup.style.left = `${(popupX * zoomLevel * wheelZoom) + panOffsetX - (parseInt(popupImage.style.width, 10) / 2) + 10}px`;
				popup.style.top = `${(popupY * zoomLevel * wheelZoom) + panOffsetY - parseInt(popupImage.style.height, 10)}px`;
			}
		}
		...
		function drawMap(manualDraw = false) {
			if (manualDraw || isDragging || isResizing || isPanning || goToTarget) {
				...
				for (let i = 0; i < rects.length; i++) {
					...
					ctx.fillStyle = "red";
					ctx.beginPath();
					ctx.arc(rect.x + rect.width, rect.y + rect.height, 5 * wheelZoom, 0, 2 * Math.PI);
					ctx.fill();
