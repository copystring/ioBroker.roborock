...
window.onload = function() {
	...
	(async () => {
		...
		function getMouseX(x) {
			return(Math.floor((((x - canvasOffsetX - panOffsetX) / wheelZoom) + mapMinX ) / scaleFactor) - 1); // - 1 because of a slight map offset
		}
		function getMouseY(y) {
			return Math.floor((((y - canvasOffsetY - panOffsetY) / wheelZoom) + mapMinY) / scaleFactor) - 1; // - 1 because of a slight map offset
		}
		...
		let offsetX, offsetY;
		const resizeSquareBorderWidth = 1;
		...
		async function downEvent(e) {
			...
			const point = await localCoordsToRobotCoords({x: mouseX, y: mouseY + 2});
			...
			if (data.map.OBSTACLES2) {
				data.map.OBSTACLES2.forEach((obstacle) => {
					...
					if (distance <= 5) {
						...
						const factor = wheelZoom / zoomLevel;
						circleXoffset = x * factor;
						circleYoffset = y * factor;
						...
						if (popupImage.src) popupImage.src = null;
		...
		canvas.addEventListener("wheel", (e) => {
			...
			wheelZoom = Math.round(Math.min(Math.max(wheelZoom, minZoom), maxZoom) * 100) / 100;

			panOffsetX = Math.round(clientX - (clientX - panOffsetX) / (mapSizeX * prevZoom) * mapSizeX * wheelZoom);
			panOffsetY = Math.round(clientY - (clientY - panOffsetY) / (mapSizeY * prevZoom) * mapSizeY * wheelZoom);
		...
		addButton.addEventListener("click", function() {
			...
			const x = Math.floor(((mapMinX + (-panOffsetX / wheelZoom)) / scaleFactor) + (canvas.width / 2 / scaleFactor / wheelZoom) - 12.5);
			const y = Math.floor(((mapMinY + (-panOffsetY / wheelZoom)) / scaleFactor) + (canvas.height / 2 / scaleFactor / wheelZoom) - 12.5);
			const width = 25;
			const height = 25;
		...
		async function drawBackgroundImage() {
			...
			image.onload = await function() {
				...
				for (let i = 0; i < pixels.length; i+=4) {
					...
					if (alpha > 0) { // Check if the alpha value is non-zero
		...
		function updateRobotZones()
		{
			...
			for (const rect in rects) {
				const zone = [];
				localCoordsToRobotCoords({x: getX(rects[rect].x), y: getY(rects[rect].y)}).then (coords1 => {
					console.log("coords1.x : " + coords1.x + " coords1.y: " + coords1.y);
					localCoordsToRobotCoords({x: getX(rects[rect].x + rects[rect].width), y: getY(rects[rect].y + rects[rect].height)}).then (coords2 => {
		...
		function updatePopupPosition() {
			if (popupTimeout) {
				const factor = wheelZoom / zoomLevel;
				const x = robotXtoPixelX(data.map.IMAGE, data.map.OBSTACLES2[0][0] / 50)/scaleFactor;
				const y = robotYtoPixelY(data.map.IMAGE, data.map.OBSTACLES2[0][1] / 50)/scaleFactor;
				const realX = (x - canvasOffsetX - 58);
				const realY = (y - canvasOffsetY - 2);
				const canvasX = (realX + 45) * factor - 50 + panOffsetX;
				const canvasY = ((realY - 22.5) * factor - 110 + panOffsetY);

				popup.style.transform = `translate(${canvasX}px, ${canvasY}px)`;
			}
		}
		...
		function drawMap(manualDraw = false) {
			if (manualDraw || isDragging || isResizing || isPanning || goToTarget) {
				...
				const targetWidth = mapSizeX * wheelZoom;
				const targetHeight = mapSizeY * wheelZoom;
				ctx.drawImage(truncatedCanvas, 0, 0, targetWidth, targetHeight);

				for (let i = 0; i < rects.length; i++) {
					const rect = rects[i];
					const rectMinX = Math.round(((rect.x * scaleFactor) - mapMinX) * wheelZoom + (2 * wheelZoom));
					const rectMinY = Math.round(((rect.y * scaleFactor) - mapMinY) * wheelZoom + (2 * wheelZoom));
