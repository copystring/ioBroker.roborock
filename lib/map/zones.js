...
window.onload = function() {
	(async () => {
		...
		function downEvent(e) {
			...
			const mouseX = getX(clientX);
			const mouseY = getY(clientY);
			...
			for (let i = 0; i < rects.length; i++) {
				...
				offsetX = clientX - rect.x + 1;
				offsetY = clientY - rect.y + 1;
				...
				if (
					...
					offsetX = clientX - rect.x - rect.width;
					offsetY = clientY - rect.y - rect.height;
					selectedRect = i;
					isResizing = true;
					e.preventDefault();
				}
		...
		function moveEvent(e) {
			...
			const clientY = e.pageY || e.touches[0].clientY;
			if (isDragging) {
				const draggingX = clientX - offsetX + 1;
				const draggingY = clientY - offsetY + 1;
				...
				rects[selectedRect].width = clientX - rects[selectedRect].x - offsetX;
				rects[selectedRect].height = clientY - rects[selectedRect].y - offsetY;
				drawRects();
			}
			else if (goToTarget)
			{
		...
		function convertToRobotZone(rect)
		{
			...
			localCoordsToRobotCoords({x: getX(rect.x + canvasOffsetX + 2), y: getY(rect.y + canvasOffsetY + 2)}).then (coords1 => {
				console.log("coords1.x : " + coords1.x + " coords1.y: " + coords1.y);
				localCoordsToRobotCoords({x: getX(rect.x + rect.width + canvasOffsetX + 2), y: getY(rect.y + rect.height + canvasOffsetY + 2)}).then (coords2 => {
