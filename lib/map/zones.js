...
window.onload = function() {
	...
	(async () => {
		...
		addButton.addEventListener("click", function() {
			...
			const x = Math.round((((mapMinX + (-panOffsetX / wheelZoom)) / scaleFactor) + (canvas.width / 2 / scaleFactor / wheelZoom) - 12.5) * 100) / 100;
			const y = Math.round((((mapMinY + (-panOffsetY / wheelZoom)) / scaleFactor) + (canvas.height / 2 / scaleFactor / wheelZoom) - 12.5) * 100) / 100;
			const width = 25;
			const height = 25;
		...
		function updatePopupPosition() {
			if (popupTimeout) {
				...
				const realX = (x - canvasOffsetX);
				const realY = (y - canvasOffsetY);
		...
		function drawMap(manualDraw = false) {
			if (manualDraw || isDragging || isResizing || isPanning) {
				...
				for (let i = 0; i < rects.length; i++) {
					const rect = rects[i];
					const rectMinX = Math.round(((rect.x * scaleFactor) - mapMinX) * wheelZoom + (3 * wheelZoom));
					const rectMinY = Math.round(((rect.y * scaleFactor) - mapMinY) * wheelZoom + (3 * wheelZoom));
