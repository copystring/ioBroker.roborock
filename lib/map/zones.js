...
window.onload = function() {
	...
	(async () => {
		...
		function getMouseX(x) {
			return (Math.round((x - canvasOffsetX - panOffsetX) / wheelZoom + mapMinX ) / scaleFactor - 1); // - 1 because of a slight map offset
		}
		function getMouseY(y) {
			return (Math.round((y - canvasOffsetY - panOffsetY) / wheelZoom + mapMinY ) / scaleFactor - 1); // - 1 because of a slight map offset
		}
