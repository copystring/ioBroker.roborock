...
window.onload = function() {
	(async () => {
		...
		function getX(x) {
			return Math.round((x - canvasOffsetX - 2));
		}
		function getY(y) {
			return Math.round((y - canvasOffsetY - 2));
		}
		...
		function convertToRobotZone(rect)
		{
			...
			localCoordsToRobotCoords({x: getX(rect.x), y: getY(rect.y)}).then (coords1 => {
				console.log("coords1.x : " + coords1.x + " coords1.y: " + coords1.y);
				localCoordsToRobotCoords({x: getX(rect.x + rect.width), y: getY(rect.y + rect.height)}).then (coords2 => {
