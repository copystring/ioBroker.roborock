...
exports.register = function (n, s) {
  module14(!o.has(n), 'Tried to register two views with the same name %s', n);
  o.set(n, s);
  return n;
};
...
exports.get = function (n) {
  ...
  if (u.has(n)) s = u.get(n);
    ...
    if ('function' != typeof l)
      module14(false, 'View config not found for name %s.%s', n, 'string' == typeof n[0] && /[a-z]/.test(n[0]) ? ' Make sure to start component names with a capital letter.' : '');
    o.set(n, null);
    f((s = l()));
    u.set(n, s);
  }
  module14(s, 'View config not found for name %s', n);
  return s;
};
