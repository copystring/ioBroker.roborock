...
var module398 = require('./398'),
  ...
  module1483 = require('./1483').getIndexPngBase64,
  module1486 = require('./1486').getCleanUpedWalls,
  module1482 = require('./1482'),
  x = module1482.convertFloorDataToImage,
  O = module1482.precessFurnitureData,
  M = {
    type: 2,
    header: 2,
    payload: 4,
...
function S(t, n, o) {
  ...
  var Q = new Array(),
    R = new Array(),
    aa = o ? module1417.colorEggIndexMap : module1417.colorIndexMap,
    ta = module1417.colorHighLightIndexMap;

  if (n) {
    aa = o ? module1417.colorEggIndexMapDark : module1417.colorIndexMapDark;
    ta = module1417.colorHighLightIndexMapDark;
  }
  ...
  for (var La in ((s.mapArea = Math.round((25 * h) / 1e4)), (s.left += f.left - 0.5), (s.top += f.top - 0.5), A))
    D[La] = 'data:image/png;base64,' + module1483(s.width, s.height, A[La]);

  for (var Ga = module1486(t), Ua = ['#DFDFDFff', '#50A4FF', '#FF744D', '#008FA8', '#F5AF10', '#E9E9E9ff'], Wa = new Array(j), qa = 0; qa < F.length; qa++) {
    var Va = F[qa];
    Wa[qa] = Ua[Va];
  }
  ...
  return u(
    u(
      {
        image: 'data:image/png;base64,' + module1483(s.width, s.height, w),
        imageNoBlock: 'data:image/png;base64,' + module1483(s.width, s.height, O),
        imageBlockCleanBackground: 'data:image/png;base64,' + module1483(s.width, s.height, M),
        image3DBackground: 'data:image/png;base64,' + module1483(s.width, s.height, T),
        image3DBackgroundDark: 'data:image/png;base64,' + module1483(s.width, s.height, I),
      },
      s
    ),
    {},
...
function F(t, n) {
  ...
  return {
    image: 'data:image/png;base64,' + module1483(3 * f.width, 3 * f.height, l),
    capData: s,
    firstIndex: u,
    capCount: p,
  };
...
function C(t, n, o) {
  ...
  if (f && undefined !== f.top) {
    ...
    return u(
      {
        image: 'data:image/png;base64,' + module1483(6 * f.width, 6 * f.height, s),
      },
      f
    );
  }
