...
var module394 = require('./394'),
  ...
  module1340 = require('./1340').getIndexPngBase64,
  z = {
    type: 2,
    header: 2,
    payload: 4,
...
function k(t, o) {
  ...
  if (f && undefined !== f.top) {
    ...
    var H = new Array(),
      J = new Array(),
      U = module1268.colorIndexMap,
      $ = module1268.colorHighLightIndexMap;

    if (o) {
      U = module1273.colorIndexMapDark;
      $ = module1273.colorHighLightIndexMapDark;
    }
    ...
    for (var Da in ((f.mapArea = Math.round((25 * n) / 1e4)), (f.left += t.left - 0.5), (f.top += t.top - 0.5), w))
      A[Da] = 'data:image/png;base64,' + module1340(f.width, f.height, w[Da]);

    delete t.data;
    return p(
      p(
        {
          image: 'data:image/png;base64,' + module1340(f.width, f.height, c),
          imageNoBlock: 'data:image/png;base64,' + module1340(f.width, f.height, M),
          imageBlockCleanBackground: 'data:image/png;base64,' + module1340(f.width, f.height, j),
        },
        f
      ),
      {},
...
function S(t, o) {
  ...
  if (f && undefined !== f.top) {
    ...
    return {
      image: 'data:image/png;base64,' + module1340(3 * f.width, 3 * f.height, s),
      capData: l,
      firstIndex: p,
      capCount: u,
    };
...
function P(t, o, n) {
  ...
  return 'data:image/png;base64,' + module1340(3 * o.width, 3 * o.height, h);
}
