...
module.exports = function (o) {
  function l(l, c, s, u, p) {
    if (c[s]) {
      var f = c[s],
        y = typeof f;
      if ('object' !== y) module14(false, 'Invalid ' + (p || '(unknown)') + ' `' + s + '` of type `' + y + '` supplied to `' + u + '`, expected `object`.');
      ...
      for (var w in v) {
        ...
        if (J) module14(false, J.message + '\nBad object: ' + JSON.stringify(c[s], null, '  '));
      }
    } else l && module14(false, 'Required object `' + s + '` was not specified in `' + u + '`.');
  }
