        ...
        var t = this,
          n = this.state.maps.map(function (n, o) {
            var s = t.props.actionPrefix
              ? module1215.FloorMapPageUtils.getRealName(n.name)
              : module510.soundpackage_update + ' ' + module1215.FloorMapPageUtils.getRealName(n.name);
            return o > 0 ? s : t.isHiddenSaveAction ? s : module1215.FloorMapPageUtils.getRealName(n.name);
          });

        switch (this.mode) {
          case k.SaveNewMapInSingleFloor:
            n = this.state.maps.map(function (t, n) {
              return 0 == n ? module1215.FloorMapPageUtils.getRealName(t.name) : module510.soundpackage_update + ' ' + module1215.FloorMapPageUtils.getRealName(t.name);
            });
        }
