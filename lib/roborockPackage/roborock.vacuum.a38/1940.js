var module50 = require('./50'),
  regeneratorRuntime = require('regenerator-runtime'),
  module7 = require('./7'),
  module9 = require('./9'),
  module11 = require('./11'),
  React = require('react'),
  module12 = require('./12'),
  module381 = require('./381'),
  module1329 = require('./1329'),
  module418 = require('./418'),
  module391 = require('./391'),
  module414 = require('./414'),
  module390 = require('./390'),
  module385 = require('./385'),
  module1411 = require('./1411'),
  module394 = require('./394'),
  module1330 = require('./1330'),
  module515 = require('./515');

function k(t, n) {
  var o = Object.keys(t);

  if (Object.getOwnPropertySymbols) {
    var s = Object.getOwnPropertySymbols(t);
    if (n)
      s = s.filter(function (n) {
        return Object.getOwnPropertyDescriptor(t, n).enumerable;
      });
    o.push.apply(o, s);
  }

  return o;
}

function T(t) {
  for (var o = 1; o < arguments.length; o++) {
    var s = null != arguments[o] ? arguments[o] : {};
    if (o % 2)
      k(Object(s), true).forEach(function (o) {
        module50.default(t, o, s[o]);
      });
    else if (Object.getOwnPropertyDescriptors) Object.defineProperties(t, Object.getOwnPropertyDescriptors(s));
    else
      k(Object(s)).forEach(function (n) {
        Object.defineProperty(t, n, Object.getOwnPropertyDescriptor(s, n));
      });
  }

  return t;
}

function V() {
  if ('undefined' == typeof Reflect || !Reflect.construct) return false;
  if (Reflect.construct.sham) return false;
  if ('function' == typeof Proxy) return true;

  try {
    Date.prototype.toString.call(Reflect.construct(Date, [], function () {}));
    return true;
  } catch (t) {
    return false;
  }
}

var module1404 = require('./1404'),
  module500 = require('./500').strings,
  module1153 = require('./1153'),
  R = (function (t) {
    module7.default(R, t);

    var module50 = R,
      module515 = V(),
      k = function () {
        var t,
          o = module11.default(module50);

        if (module515) {
          var s = module11.default(this).constructor;
          t = Reflect.construct(o, arguments, s);
        } else t = o.apply(this, arguments);

        return module9.default(this, t);
      };

    function R(t) {
      var n;
      module4.default(this, R);

      (n = k.call(this, t)).onOverTimer = function () {
        globals.showToast(module500.map_object_ignore_failed);
      };

      n.mapCarpetFocus = function (t, o) {
        n.editAction = o ? module1330.EditAction.Ignore : module1330.EditAction.AddCarpet;
        n.setState(
          o
            ? {
                cusCarpetEdited: false,
              }
            : {
                ignoreCarpetEdited: false,
              }
        );
        n.showEditMenu(o, t);
      };

      n.resetAllCarpetStatus = function (t) {
        n.editAction = '';
        n.setState(
          {
            ignoreCarpetEdited: false,
            cusCarpetEdited: false,
          },
          function () {
            var o, s, p;
            if (!(null == (o = n.mapView))) o.clearCarpetFbzFocus(false);
            if (!(null == (s = n.mapView))) s.clearCarpetFbzFocus(true);
            if (!(null == (p = n.editMenu)))
              p.setState({
                tip: module500.map_edit_carpet_init_tip,
              });
            if (!t) n.endLoading();
            if (t) t();
          }
        );
      };

      n.mapCarpetSaveEdit = function (t, o, s, p) {
        if (s) n.ignoreFocusId = t;
        else n.addFocusId = t;
        if (s) n.trySavingChanges(o, p);
        else n.trySavingAddChanges(o);
      };

      n.onPressAddCarpetButton = function () {
        var t;
        return regeneratorRuntime.default.async(
          function (o) {
            for (;;)
              switch ((o.prev = o.next)) {
                case 0:
                  if (null == (t = n.mapView) ? undefined : t.isCarpetReachMaxNum(true)) globals.showToast(module500.map_edit_carpet_zone_exceed_limit);
                  else n._onPressAddButton(true);

                case 1:
                case 'end':
                  return o.stop();
              }
          },
          null,
          null,
          null,
          Promise
        );
      };

      n.state = {
        mapStatus: module381.RSM.mapStatus,
        ignoreCarpetEdited: false,
        cusCarpetEdited: false,
      };
      n.editAction = '';
      return n;
    }

    module5.default(R, [
      {
        key: 'UNSAFE_componentWillMount',
        value: function () {
          var t = this;
          this.backListener = module12.BackHandler.addEventListener(module12.BackHandler.DEVICE_BACK_EVENT, function () {
            t.onNavigationBackPress();
            return true;
          });
          this.carpetRect = this.props.navigation.getParam('carpetRect', null);
          this.carpetEditMode = this.props.navigation.getParam('action', module381.CarpetEditMode.CarpetIgnore);
        },
      },
      {
        key: 'componentDidMount',
        value: function () {
          return regeneratorRuntime.default.async(
            function (t) {
              for (;;)
                switch ((t.prev = t.next)) {
                  case 0:
                    this.configNavibar();
                    this.initMapView();
                    this.registListeners();
                    module1329.MM.getMap(true);

                  case 4:
                  case 'end':
                    return t.stop();
                }
            },
            null,
            this,
            null,
            Promise
          );
        },
      },
      {
        key: 'componentWillUnmount',
        value: function () {
          this.removeListeners();
        },
      },
      {
        key: 'configNavibar',
        value: function () {
          module1330.MapEditCommonService.setNavigation(this, null, this.onNavigationBackPress.bind(this));
        },
      },
      {
        key: 'initMapView',
        value: function () {
          if (this.mapView) {
            var t = module1329.MM.mapData;
            this.mapView.setState(T({}, t));
          }
        },
      },
      {
        key: 'registListeners',
        value: function () {
          var t = this;
          this.mapListener = module12.DeviceEventEmitter.addListener(module418.NotificationKeys.MapDidUpdate, function (n) {
            if (t.mapView)
              t.mapView.setState(
                T(
                  T({}, module1329.MM.mapData),
                  {},
                  {
                    robotStatus: module381.RSM.state,
                  }
                )
              );

            if (t.waitForUpdate) {
              t.resetAllCarpetStatus(null);
              t.waitForUpdate = false;
            }
          });
          this.robotStatusListener = module12.DeviceEventEmitter.addListener(module418.NotificationKeys.RobotStatusDidUpdate, function (n) {
            t.setState({
              mapStatus: module381.RSM.mapStatus,
            });
          });
          this.mapEditDidChange = module12.DeviceEventEmitter.addListener(module418.NotificationKeys.MapEditDidChange, function (n) {
            if (!(n.data != module1330.FbzType.FBZ_TYPE_CARPET && n.data != module1330.FbzType.RECT_TYPE_CARPET))
              t.setState({
                ignoreCarpetEdited: n.data == module1330.FbzType.FBZ_TYPE_CARPET,
                cusCarpetEdited: n.data == module1330.FbzType.RECT_TYPE_CARPET,
              });
          });
        },
      },
      {
        key: 'removeListeners',
        value: function () {
          if (this.mapListener) this.mapListener.remove();
          if (this.backListener) this.backListener.remove();
          if (this.robotStatusListener) this.robotStatusListener.remove();
          if (this.mapEditDidChange) this.mapEditDidChange.remove();
        },
      },
      {
        key: 'onNavigationBackPress',
        value: function () {
          this.quit();
        },
      },
      {
        key: 'quit',
        value: function () {
          return regeneratorRuntime.default.async(
            function (t) {
              for (;;)
                switch ((t.prev = t.next)) {
                  case 0:
                    t.prev = 0;
                    t.next = 3;
                    return regeneratorRuntime.default.awrap(module414.default.endEditMap());

                  case 3:
                    t.next = 8;
                    break;

                  case 5:
                    t.prev = 5;
                    t.t0 = t.catch(0);
                    console.log('end carpet map error: ' + ('object' == typeof t.t0 ? JSON.stringify(t.t0) : t.t0));

                  case 8:
                    this.props.navigation.pop();

                  case 9:
                  case 'end':
                    return t.stop();
                }
            },
            null,
            this,
            [[0, 5]],
            Promise
          );
        },
      },
      {
        key: 'trySavingAddChanges',
        value: function (t) {
          var n, o, s;
          if (t && (null == (n = this.mapView) ? undefined : n.isCustomCarpetOverlap())) globals.showToast(module500.map_edit_carpet_overlap_tip);
          else if (t && (null == (o = this.mapView) ? undefined : o.isCustomCarpetRectUnknown())) globals.showToast(module500.map_edit_carpet_unknown_tip);
          else {
            var p = null == (s = this.mapView) ? undefined : s.getCarpetZonesParams(true, t ? -1 : this.addFocusId);
            if (null == p) p = [];
            this.saveChangedParams(p);
          }
        },
      },
      {
        key: 'trySavingChanges',
        value: function (t, n) {
          var o,
            s,
            p = this;
          if (t && (null == (o = this.mapView) ? undefined : o.isIgnoreCarpetOverlap())) globals.showToast(module500.map_edit_carpet_overlap_tip);
          else {
            var u = null == (s = this.mapView) ? undefined : s.getCarpetZonesParams(false, t ? -1 : this.ignoreFocusId);
            if ((null == u && (u = []), t && !n)) this.saveChangedParams(u);
            else {
              var c,
                l = t ? module500.map_edit_carpet_ignore_confirm_ignore : module500.map_edit_carpet_ignore_confirm_cancel,
                h = {
                  text: module500.localization_strings_Main_MainPage_11,
                },
                f = {
                  text: module500.rubys_location_confirm_button_confirm,
                  onPress: function () {
                    return p.saveChangedParams(u);
                  },
                };
              if (!(null == (c = globals.Alert))) c.alert(l, '', [h, f]);
            }
          }
        },
      },
      {
        key: 'saveChangedParams',
        value: function (t) {
          var n = module381.RSM.currentMapId;
          if (module390.default.isMultiFloorSupported())
            this.state.mapStatus != module381.MapStatus.None && module1329.MM.maps.length >= 1 && -1 == n
              ? globals.showToast(module500.no_map_status_description)
              : 1 == module1329.MM.maps.length && 1 == module1329.MM.mapCountMax
              ? ((n = module1329.MM.maps[0].id), this.startMapSave(n, t))
              : this.startMapSave(n, t);
          else this.startMapSave(n, t);
        },
      },
      {
        key: 'startMapSave',
        value: function (t, n) {
          var o = this;
          module1404.setTimeout(function () {
            return o.startLoading();
          }, 300);
          if (this.editAction == module1330.EditAction.Ignore)
            module1404.setTimeout(function () {
              return module1329.MM.saveCarpetIgnoreZone(t, n, o.onMapSaveCompleted.bind(o));
            }, 500);
          else if (this.editAction == module1330.EditAction.AddCarpet)
            module1404.setTimeout(function () {
              return module1329.MM.saveCarpetAddedZone(t, 1, n, o.onMapSaveCompleted.bind(o));
            }, 500);
        },
      },
      {
        key: 'onMapSaveCompleted',
        value: function (t) {
          var n = this;

          if (t) {
            module1329.MM.getMap(true);
            this.waitForUpdate = true;
            module1404.setTimeout(function () {
              if (n.waitForUpdate) {
                n.resetAllCarpetStatus(null);
                n.waitForUpdate = false;
              }
            }, 2e4);
          } else {
            globals.showToast(module500.robot_communication_exception);
            this.endLoading();
          }
        },
      },
      {
        key: '_onPressAddButton',
        value: function (t) {
          var n;
          return regeneratorRuntime.default.async(
            function (s) {
              for (;;)
                switch ((s.prev = s.next)) {
                  case 0:
                    this.editAction = module1330.EditAction.AddCarpet;
                    if (!(null == (n = this.mapView))) n.addCarpetNewZone(t);
                    this.showEditMenu(false, true);
                    s.prev = 3;
                    s.next = 6;
                    return regeneratorRuntime.default.awrap(module414.default.startEditMap());

                  case 6:
                    s.next = 11;
                    break;

                  case 8:
                    s.prev = 8;
                    s.t0 = s.catch(3);
                    console.log('add new czone: ' + ('object' == typeof s.t0 ? JSON.stringify(s.t0) : s.t0));

                  case 11:
                  case 'end':
                    return s.stop();
                }
            },
            null,
            this,
            [[3, 8]],
            Promise
          );
        },
      },
      {
        key: 'showEditMenu',
        value: function (t, n) {
          var o,
            s = '';
          s = t
            ? n
              ? module500.map_edit_carpet_ignore_tip
              : module500.map_edit_carpet_ignore_edit_tip
            : n
            ? module500.map_edit_carpet_add_tip
            : module500.map_edit_carpet_add_edit_tip;
          if (!(null == (o = this.editMenu)))
            o.setState({
              tip: s,
            });
        },
      },
      {
        key: 'startLoading',
        value: function () {
          var t;
          if (!(null == (t = this.loadingView))) t.showWithText(module500.rubys_main_diag_update_map);
        },
      },
      {
        key: 'endLoading',
        value: function () {
          var t;
          if (!(null == (t = this.loadingView))) t.hide();
        },
      },
      {
        key: '_getMapView',
        value: function () {
          var t = this,
            n = React.default.createElement(module1330.MapView, {
              ref: function (n) {
                t.mapView = n;
              },
              parent: this,
              style: module1330.MapEditCommonStyles.map,
              top: module1330.MapEditCommonStyles.mapTop,
              bottom: module1330.MapEditCommonStyles.mapBottom,
              left: module1330.MapEditCommonStyles.mapLeft,
              right: module1330.MapEditCommonStyles.mapRight,
              initFBZone: this.carpetRect,
              mapMode: module1153.MAP_MODE_CARPET_EDIT,
              subCarpetMode: this.carpetEditMode,
              hideAccessory: true,
              mapCarpetFocusCallback: this.mapCarpetFocus,
              mapCarpetSaveEdit: this.mapCarpetSaveEdit,
              ignoreCarpetEdited: this.state.ignoreCarpetEdited,
              cusCarpetEdited: this.state.cusCarpetEdited,
              resetAllCarpetStatus: this.resetAllCarpetStatus,
            }),
            o = React.default.createElement(module1411.default, {
              tip: module500.no_map_tip,
            });
          return this.state.mapStatus == module381.MapStatus.None ? o : n;
        },
      },
      {
        key: '_getMapEditMenuView',
        value: function () {
          var t = this;
          return React.default.createElement(module1330.MapEditMenuView, {
            type: module1330.MenuType.Menu_Carpet,
            subType: this.carpetEditMode,
            ref: function (n) {
              return (t.editMenu = n);
            },
            hasConfirmMenu: true,
            onPressAddCarpetButton: this.onPressAddCarpetButton,
          });
        },
      },
      {
        key: 'render',
        value: function () {
          var t = this,
            n = this.context.theme;
          return React.default.createElement(
            module12.View,
            {
              style: [
                module1330.MapEditCommonStyles.root,
                {
                  height: module394.default.sharedCache().ScreenHeight - module391.default.statusbarHeight(),
                  backgroundColor: n.mapEdit.backgroundColor,
                },
              ],
            },
            this._getMapView(),
            this._getMapEditMenuView(),
            React.default.createElement(module385.CancelableLoadingView, {
              loadingAccessibilityLabelKey: 'map_carpet_ignore_view_loading',
              closeAccessibilityLabelKey: 'map_carpet_ignore_view_loading_close',
              ref: function (n) {
                t.loadingView = n;
              },
              onOverTimer: this.onOverTimer,
              showButton: true,
            })
          );
        },
      },
    ]);
    return R;
  })(React.default.Component);

exports.default = R;
R.contextType = module515.AppConfigContext;
