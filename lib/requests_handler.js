...
class requests_handler {
	...
	async initTCP() {
		const locallyDiscoveredDevices = await this.local_api.getLocalDevices(); // this discovers devices in local network via udp discovery

		Object.entries(locallyDiscoveredDevices).forEach(([duid, ip]) => {
			...
			switch (parameter) {
				case "get_network_info":
					...
					for (const attribute in networkInfo) {
						if (attribute == "ip" && !(await this.isRemoteDevice(duid))) {
					...
					for (const attribute in this.cached_get_status_value[duid][0]) {
						...
						switch (attribute) {
							...
							case "dss":
								...
								const dockingStationStatus = await this.getDockingStationStatus();
	...
	async getMap(duid) {
		if (this.adapter.config.enable_map_creation) {
			this.adapter.log.debug(`Requesting new map`);
	...
	getSelectedMap(duid) {
		...
		return this.cached_get_status_value[duid][0].map_status >> 2; // to get the currently selected map perform bitwise right shift
	}

	async sendRequest(duid, method, params, secure = false, photo = false) {
		const remoteConnection = await this.isRemoteDevice(duid);
		...
		const payload = this.message_parser.buildPayload(protocol, messageID, method, params, secure, photo);
	...
	async isRemoteDevice(duid) {
		...
		const remoteDevice = this.local_api.remoteDevices.has(duid);
