...
class requests_handler {
	...
	async sendRequest(duid, method, params) {
		...
		this.adapter.log.debug(`sendRequest version: ${version}`);
		const robotModel = this.adapter.http_api.getRobotModel(duid);
		this.adapter.log.debug(`sendRequest robotModel: ${robotModel}`);
		let protocol = 101;
		...
		return new Promise((resolve, reject) => {
			if (!deviceOnline) {
				...
				} else if (robotModel == "roborock.vacuum.a144" || robotModel == "roborock.vacuum.a147" || robotModel == "roborock.vacuum.a187" || robotModel == "roborock.vacuum.a156") { // Special case devices with version L01
					this.mqtt_api.sendMessage(duid, roborockMessage);
					this.adapter.log.debug(`Sent payload for ${duid} with ${payload} using cloud connection for L01 devices`);
				} else {
