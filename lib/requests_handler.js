					...
					case "get_network_info":
						...
						for (const attribute in value) {
							...
							if (this.adapter.isInitializing) {
								// await this.adapter.setObjectNotExistsAsync(`Devices.${duid}.networkInfo.${attribute}`);
								const objectType = attribute == "rssi" ? "number" : "string";
								await this.adapter.createStateObjectHelper(`Devices.${duid}.networkInfo.${attribute}`, attribute, objectType, null, null, "value", true, false);
							}
							this.adapter.setStateAsync(`Devices.${duid}.networkInfo.${attribute}`, { val: value[attribute], ack: true });
						}
						break;
					case "get_consumable":
