...
class requests_handler {
	...
	async sendRequest(duid, method, params) {
		...
		const model = await this.adapter.getDeviceModel(duid);
		let protocol = 101;
		...
		return new Promise((resolve, reject) => {
			if (!deviceOnline) {
				...
				} else if (model == "roborock.vacuum.a144" || model == "roborock.vacuum.a147" || model == "roborock.vacuum.a187" || model == "roborock.vacuum.a156") { // Special case devices with version L01
					this.mqtt_api.sendMessage(duid, roborockMessage);
				} else {
