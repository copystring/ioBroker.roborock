					...
					case "get_prop":
						if (attribute == "get_status") {
							...
							for (const attribute in this.cached_get_status_value[duid][0]) {
								...
								this.adapter.ensureState(`Devices.${duid}.deviceStatus.${attribute}`, { val: this.cached_get_status_value[duid][0][attribute], ack: true });
							}
						}
						break;
					...
					case "get_multi_maps_list":
						...
						for (const mapParameter in value[0]) {
							if (typeof value[0][mapParameter] === "number") {
								...
								this.adapter.setStateAsync(statePath, { val: value[0][mapParameter], ack: true });
							}
						}
