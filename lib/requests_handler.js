...
class requests_handler {
	...
	async initTCP() {
		this.localDevices = await this.local_api.getLocalDevices(); // this discovers devices in local network via udp discovery
	...
	isLocalDevice(duid) {
		// const localDevice = this.localDevices.find((device) => device.duid == duid);
		// const localDevice = this.localDevices.has(duid);
