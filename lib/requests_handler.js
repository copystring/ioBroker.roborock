...
class requests_handler {
	...
	async initTCP() {
		const tcpDevices = await this.local_api.getLocalDevices(); // discovers devices in local network via UDP discovery
		this.localDevices = { ...this.localDevices, ...tcpDevices }; // this merges devices found via get_network_info with tcpDevices
	...
	async getStatus(duid) {
		...
		switch (productCategory) {
			...
			case "robot.vacuum.cleaner":
				await this.getParameter(duid, "get_prop", ["get_status"]);
				break;
		}
	}
		...
		try {
			...
			for (const cleaningAttribute in cleaningAttributes) {
				...
				if (["clean_time", "clean_area", "clean_count"].includes(mappedAttribute)) {
					await this.adapter.ensureState(`Devices.${duid}.cleaningInfo.${cleaningAttribute}`, {
					...
					for (const cleaningRecord in cleaningAttributes[cleaningAttribute]) {
						...
						for (const cleaningRecordAttribute in cleaningRecordAttributes) {
							...
							await this.adapter.ensureState(`Devices.${duid}.cleaningInfo.records.${cleaningRecord}.${mappedRecordAttribute}`, {
						...
						if (this.adapter.config.enable_map_creation == true) {
							await this.adapter.ensureFolder(`Devices.${duid}.cleaningInfo.records.${cleaningRecord}.map`);
	...
	async getParameter(duid, parameter, attribute) {
		...
		this.sendRequest(duid, parameter, attribute)
			.then(async (value) => {
				switch (parameter) {
					...
					case "get_prop":
						if (attribute == "get_status") {
							...
							for (const attribute in this.cached_get_status_value[duid][0]) {
								...
								const commonExtended = this.adapter.device_features.getCommonExtended(attribute);
								this.adapter.ensureState(`Devices.${duid}.deviceStatus.${attribute}`, { val: this.cached_get_status_value[duid][0][attribute], ack: true }, commonExtended);
							}
						}
						break;
					...
					default: {
						if (parameterFolders[parameter]) {
							...
							this.adapter.setStateAsync(`Devices.${duid}.${targetFolder}.${mode}`, { val: value[0], ack: true });
						} else {
							// Unknown parameter
			...
			.catch((error) => {
				if (error) {
					this.adapter.log.error(`Failed to process getParameter ${parameter} with attribute ${attribute} for ${duid}. ${error?.stack || error}`);
				}
			});
	}
	...
	isCleaning(duid) {
		...
		if (!this.cached_get_status_value[duid]) {
			throw new Error(
				`this.cached_get_status_value for ${duid} is not initialized. Request get_status first. this.cached_get_status_value[${duid}]: ${JSON.stringify(
					this.cached_get_status_value[duid]
				)}`
			);
		}
		...
		if (roborockMessage) {
			if (version == "A01") {
				if (!deviceOnline) {
					...
					return; // No need to reject here. A01 devices don't support promises
				}
