...
class mqtt_api {
					...
					client.on("connect", (result) => {
						if (result) {
							client.subscribe(`rr/m/o/${rriot.u}/${this.mqttUser}/#`, (err, granted) => {
								if (err) {
									reject(new Error(`Failed to subscribe to Roborock MQTT Server! Error: ${err}, granted: ${JSON.stringify(granted)}`));
								}
							});
			...
			const attemptConnection = async () => {
				while (retryCount < maxRetries) {
					try {
						await Promise.race([
							connectToMQTT(),
							new Promise((_, reject) => setTimeout(() => reject(new Error(`Connection timed out after ${retryDelay}ms`)), retryDelay)),
						]);
						return resolve();
					} catch (error) {
						retryCount++;
						...
						if (retryCount >= maxRetries) {
							return reject(new Error(`Failed to connect to MQTT after ${maxRetries} attempts. ${error.stack}`));
						}
					}
				}
			};
	...
	async ensureEndpoint() {
		...
		try {
			...
			this.adapter.log.catchError(`Error ensuring endpoint: ${error.stack}`);
		}
	}
